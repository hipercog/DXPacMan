...
double calculateState( Tree<GhostMoves> *node_state )
{
	...
	if(gM.probs > 0)
	{
		...
		for( int count = 0; count < POT_GS; count++ )
		{
			...
			{
				ghostProbs += iter_GstMv.m_node->m_data.probs;
				iter_GstMv.Up();
			}
			sum += utility(S) / ghostProbs;
			S = tempPS;
		}
	}
...
//	int factor = (int)pow( 2.0, NUM_GS );
	...
	{
		int dir = -1;
		for( int cmbn = 0; cmbn < POT_GS; cmbn++ ) //(int)pow( 4.0, NUM_GS-1 )
		{
			if( cmbn%(int)pow( 4.0, g ) == 0 )
				dir++;
			//Check around ghosts
			checkAroundGhost(dir%4, g, baseGhostX[g], baseGhostY[g], cmbn, Map);
		}
	}
...
void printGhostMoves()
{
	...
	print("\n#P(S): 1/"); sprintf_s(out, 99, "%d", GhostMove.probs); print(out); print("\n\t");
