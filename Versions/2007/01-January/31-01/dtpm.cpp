...
/**
 * Description: Utility of Pills is proximity of one Pill to Pacman, then utilityGhostHunt()
 * Arguments:	Pacman x and y 
 *				Ghost 1-4 x and y
...
{
	double util = 0.0;
	//IF a Pill is not currently active...
	if(GhostAttack)
	{
		if(NumPills == 0)
			return 0;
...
println("Ghosts Attack");
		...
		int locBound = 4;
		int locale = localGhosts(xPac, yPac, locBound);
		if( locale > 1 )
		{
			...
			if( a_Star(xPac, yPac, 20-xPac, 20-yPac, true) < locBound )
				return -100.0; //no way out! Check this
		}else
			util += 1.0 / avgDistribution(xGhost, yGhost);
	}else{
println("Pac Attack");
		util += utilityGhostHunt(xPac, yPac, xGhost, yGhost, risk) / avgDistribution(xGhost, yGhost);
	}
...
/**/
	itoa(iter, out, 10); print("path length: "); println(out); 
	for(int i = 0; i < iter; i++)
	{
		print("Iter: ");
