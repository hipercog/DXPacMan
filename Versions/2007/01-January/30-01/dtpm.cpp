	...
	for(int i = 0; i < 20; i++)
		for(int j = 0; j < 20; j++)
			if(PacMap[i][j] == 4)
			{
				...
				util -= 1 / manhattanDist(xPac, yPac, i, j);
			}
...
/**
 * Description: Utility of Pills is proximity of Ghosts to Pacman
 * Arguments:	Pacman x and y 
 *				Ghost 1-4 x and y
 */
double utilityPills(int xPac, int yPac, int *xGhost, int *yGhost /*[Ghost 1-4][x-y]*/, int risk)
{
	if(NumPills == 0)
		return 0;
	double util = NumPills * 50;
	deque <int> mDist; //Deque of distances between sprites
...
//PRINT OUTS
//itoa(xGhostCentroid, out, 10); print("\nxGhostCentroid: "); print(out);
//itoa(yGhostCentroid, out, 10); print("\t\tyGhostCentroid: "); println(out);
...
itoa(xPill[mDist[0]], out, 10); print("xPill: "); print(out);
itoa(yPill[mDist[0]], out, 10); print("\t\t\tyPill: "); println(out);

	aS = a_Star(xPac, yPac, xPill[mDist[0]], yPill[mDist[0]], false);
	if( aS == 0 )
		aS++;
	else{
itoa(aPathX[0], out, 10); print("aPathX[0]: "); print(out);
itoa(aPathY[0], out, 10); print("\t\t\taPathY[0]: "); println(out);
int index = aPathX.size()-1;
itoa(aPathX[index], out, 10); print("aPathX[end]: "); print(out);
itoa(aPathY[index], out, 10); print("\t\t\taPathY[end]: "); println(out);
	}
print("\nDist: "); itoa(aS, out, 10); print(out);
	//Utility is heavily influenced by initial proximity of Pacman to Pill - this is a counter-weight
	inc = 10.0 * (double)aS;
cout << "Util 1:" << inc << endl;
print("\nUtil: "); gcvt(inc, 5, out); println(out);
	util -= inc;
...
print("\nDist: "); itoa(aS, out, 10); print(out);
	...
	inc = 100 / (double)aS;
cout << "Util 2:" << inc << endl;
print("\nUtil: "); gcvt(inc, 5, out); println(out);
	util += inc;

cout << "RISK: " << risk << endl;
	//Utility of catching later ghosts is more heavily penalised by distance and risk
	for(int i = 1; i < 4; i++)
	{
		...
		if( aS == 0 )
			aS++;
print("\nDist: "); itoa(aS, out, 10); print(out);
		inc = (pow(2, i) * 100) / ((double)aS * (i + risk));
cout << "Util ghost:" << inc << endl;
print("\nUtil: "); gcvt(inc, 5, out); println(out);
		util += inc;
	}
/**/
...
double utilityGhosts(int xPac, int yPac, int *xGhost, int *yGhost /*[Ghost 1-4][x-y]*/, int risk)
{
	...
	if( locale > 1 )
	{
		...
		if( a_Star(xPac, yPac, 20-xPac, 20-yPac, true) < 4 )
...
/**/
	...
	if( pacTravelling )
		limit++;
	if( PacMap[src_x1][src_y1] >= limit )
	{
		aPathX.pop_front();
		aPathY.pop_front();
	}else if( PacMap[trg_x2][trg_y2] >= limit )
	{
		aPathX.pop_back();
		aPathY.pop_back();
	}
