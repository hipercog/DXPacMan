...
//  Arguments:      None
//  Return Value:   The depth of the tree from the current node to the Root - may not be true depth if current node is not a leaf or tree is asymmetric
// ----------------------------------------------------------------
    int Root()
    {
		int depth = 0;
...
// ----------------------------------------------------------------
template <class DataType>
bool PreorderToDepth( Tree<DataType>* p_node, int (*p_process)(Tree<DataType>*), int depth )
{
	int check_depth = 0;

	//To stop recursing - if depth is reached, only call p_process and don't call Preorder on the spawned children
	TreeIterator <DataType> iter_DpChk = p_node;
	check_depth = iter_DpChk.Root();

//print("\nDEPTH: "); sprintf_s(out, 99, "%d", check_depth); println(out);
    // process the current node
    if( p_process( p_node ) == 0 )
		return false;
	else if( check_depth < depth )
	{
	    // loop through each child and process it
		DListIterator<Tree<DataType>*> itr = p_node->m_children.GetIterator();
		for( itr.Start(); itr.Valid(); itr.Forth() )
			PreorderToDepth( itr.Item(), p_process, depth );
	}
	return true;
}
...
// ----------------------------------------------------------------
template <class DataType>
double PreorderAtDepth( Tree<DataType>* p_node, double (*p_process)(Tree<DataType>*), int depth )
{
	double retval = 0.0;
	int check_depth = 0;
	...
	// process the current node if the depth is right...
	if( check_depth == depth )
		retval += p_process( p_node );

    // loop through each child and process it
    DListIterator<Tree<DataType>*> itr = p_node->m_children.GetIterator();
    for( itr.Start(); itr.Valid(); itr.Forth() )
        retval += PreorderAtDepth( itr.Item(), p_process, depth );
