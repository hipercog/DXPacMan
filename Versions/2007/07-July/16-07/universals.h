...
extern int	PACMANSPRITE_START_POS_Y;		//Start position y offset for PacMan sprite
extern int	CHERRYSPRITE_START_POS_Y;		//Start position y offset for the cherry sprite
...
public:
	...
	int xPac, yPac, Lives, Points, Pills, Dots, MovedDir;
	 ...
	 */
	PacState::PacState(int PacX, int PacY, int numLives, int numPoints, int numPills, int numDots, int moved, int *ghostX, int *ghostY, bool cherryEaten, bool PacAtk, int Map[20][20])
	{
		xPac = PacX;
		yPac = PacY;
		Lives = numLives;
