...
int actions( int depth, int lookahead );
void updateState();
double calculateState( Tree<GhostMoves> *node_state );
...
int checkAroundGhost(int dir, int gstNum, int gstNumX, int gstNumY, int count, int Map[20][20]);
...
/**/
	...
	while( i < csv.size() )
	{
		...
		for( int x = 0; x < 20; x++ )
			for( int y = 0; y < 20; y++ )
			{
				tempMap[x][y] = csv[i+addend+8+count];
				count++;
			}
		S = PacState(csv[i+1], csv[i+2], csv[i+addend+2], csv[i+addend+4], csv[i+addend+3], csv[i+addend+1], csv[temp+addend+8], xGst, yGst, csv[i+addend+5], csv[i+addend+6], tempMap );
		i = ++temp;
/**/	
		buildTreeOfGhostMoves( lookahead, -1, -1, S );
		iter_GstMv.Root();
		pred = actions( lookahead, lookahead );

		sprintf_s( out, 99, "%d", csv[i] );
		print(out); print(",Prediction:,");
		
		sprintf_s( out, 99, "%d", pred );
		print(out); print(",");

		sprintf_s( out, 99, "%d", S.MovedDir );
		print(out); print(",");
		...
		fflush(fileout);
/**/		
		temptime = temptime - GetTickCount();
		cout << "Prediction " << csv[i] << ": " << pred << "\tTime: " << temptime << endl;
/**/
	}
/**/
}
...
int actions( int depth, int lookahead )
{
	...
	if( lookahead >= 0 )
	{
		...
		if( S.dtpMap[S.xPac][S.yPac - 1] > 2 )
		{
			...
			S.yPac--;
			updateState();
			actArr[0] = PreorderAtDepth( tree_GstMv, calculateState, depth - lookahead );
			actArr[0] += actions( depth, --lookahead );
			lookahead++;
			S = tempPS;
		}
		if( S.dtpMap[S.xPac][S.yPac + 1] > 2 )
		{
			...
			S.yPac++;
			updateState();
			actArr[1] = PreorderAtDepth( tree_GstMv, calculateState, depth - lookahead );
			actArr[1] += actions( depth, --lookahead );
			lookahead++;
			S = tempPS;
		}
		if( S.dtpMap[S.xPac + 1][S.yPac] > 2 )
		{
			...
			S.xPac++;
			updateState();
			actArr[2] = PreorderAtDepth( tree_GstMv, calculateState, depth - lookahead );
			actArr[2] += actions( depth, --lookahead );
			lookahead++;
			S = tempPS;
		}
		if( S.dtpMap[S.xPac - 1][S.yPac] > 2 )
		{
			...
			S.xPac--;
			updateState();
			actArr[3] = PreorderAtDepth( tree_GstMv, calculateState, depth - lookahead );
			actArr[3] += actions( depth, --lookahead );
			lookahead++;
			S = tempPS;
		}
		dir = highest( actArr, 4 );
		aVal = actArr[ dir ] / openDirs;
	}
	return dir;
}
...
double calculateState( Tree<GhostMoves> *node_state )
{
	...
	GhostMoves gM = node_state->m_data;
	if(gM.perms > 0)
	{
		iter_GstMv = node_state;
		// Generate the set of ghost move possibilities here, then calculate utility for each state. Probability follows from GhostMoves.perms
		while( iter_GstMv.Valid() )
		{
			ghostProbs += iter_GstMv.m_node->m_data.perms;
			iter_GstMv.Up();
		}
...
/**/
	...
	if(!PreorderToDepth( tree_GstMv, countSpawn, recurseLimit ))
		return false;
...
double probabilityOfGhost(int *baseGhostX, int *baseGhostY /*[Ghost 1-4][x-y]*/, int Map[20][20])
{
	...
	for(int p1 = 0; p1 < NUM_GS; p1++)
		for(int p2 = 0; p2 < POT_GS; p2++)
		{
			...
			GhostMove.ghostXYMove[p1][2][p2] = 0;
		}
	...
	for( int g = 0; g < NUM_GS; g++ )
	{
		...
		for( int cmbn = 0; cmbn < POT_GS; cmbn++ )
		{
			if( cmbn%(int)pow( 4.0, g ) == 0 )
				dir++;
			//Check around ghosts
			gDirs[g] += checkAroundGhost(dir%4, g, baseGhostX[g], baseGhostY[g], cmbn, Map);
		}
	}

	//Initialise the probabilities of ghost positions given their movement driver
	int dirCnt = 0;
	deque <int> gToP;
	deque <int> pFor1Gst;
	for( int g = 0; g < NUM_GS; g++ )
	{
		for( int i = 0; i < POT_GS; i++ )
			if( GhostMove.ghostXYMove[0][0][i] != NOMV )
				gToP.push_back( GhostMove.ghostXYMove[g][2][i] );

		pFor1Gst.push_back( gToP[0] );
		for( int i = 1; i < gToP.size(); i++ )
		{
			dirCnt = 0;
			for( int c = 0; c < pFor1Gst.size(); c++ )
				if( gToP[i] == pFor1Gst[c] )
				{
					dirCnt = 1;
					break;
				}
			if( dirCnt == 0 )
				pFor1Gst.push_back( gToP[i] );
		}

		gToP = pFor1Gst;
		dirCnt = gToP.size();
		//Find the shortest open direction toward Pacman and thus the probabilities of each permutation.
		int nearProb = 0, farProb = 0;
		if( S.PacAttack )
		{
			nearProb = 5;
			farProb = 9;
		...
		pFor1Gst[lowest( gToP )] = nearProb - dirCnt;
		pFor1Gst[highest( gToP )] = farProb - dirCnt;

		if( dirCnt > 2 )
			for( int i = 0; i < pFor1Gst.size(); i++ )
				if( pFor1Gst[i] == 0 )
					pFor1Gst[i] = 2;
		
		for( int i = 0; i < POT_GS; i++ )
		{
			for( int p = 0; p < pFor1Gst.size(); p++ )
				if( GhostMove.ghostXYMove[g][2][i] == gToP[p] )
					GhostMove.ghostXYMove[g][2][i] = pFor1Gst[p];
		}
		gToP.clear();
		pFor1Gst.clear();
	}
...
int checkAroundGhost(int dir, int gstNum, int gstNumX, int gstNumY, int count, int Map[20][20])
{
	...
	case 0:
		if( Map[gstNumX][gstNumY - 1 ] == 1 )
			GhostMove.ghostXYMove[0][0][count] = NOMV;
		else{
			retval++;
			GhostMove.ghostXYMove[gstNum][2][count] = myHeuristicDist( GhostMove.ghostXYMove[gstNum][0][count], --GhostMove.ghostXYMove[gstNum][1][count], S.xPac, S.yPac );
		}break;
	case 1:
		if( Map[gstNumX][gstNumY + 1 ] == 1 )
			GhostMove.ghostXYMove[0][0][count] = NOMV;
