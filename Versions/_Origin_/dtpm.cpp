...
/**/
	...
	while( i < csv.size() )
	{
		...
		S = PacState(csv[i+1], csv[i+2], csv[i+addend+2], csv[i+addend+4], csv[i+addend+3], csv[i+addend+1], csv[temp+addend+7], csv[i+addend+8], xGst, yGst, csv[i+addend+5] == 1, csv[i+addend+6] == 1, tempMap );
		i = ++temp;
/**/	
		buildTreeOfGhostMoves( lookahead, -1, -1, S );
		iter_GstMv.Root();
