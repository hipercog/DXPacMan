...
////////////////////////////////////////////////////////////////////////////////////////////
FILE *testfileout = fopen("evalData/printout.txt", "wt");
int PacMap[20][20];
int Score;
int NumPills = 180;
int GameLives = 5;
...
HRESULT GameLoop()
{
	...
	static int LoopCount2	= 1;
	static int LoopCount3	= 1;

	//Initialise sprite animation variables
	static BOOL bIncrease = TRUE;
	static int CurrentSprite = 0;

	//Initialise local vars holding location indices
	static FLOAT xPosInitial, yPosInitial = 0.0f;
	static int xPosIndex, yPosIndex = 0;

	//Move PacMan depending on current game loop count 
	//Note: This is a clumsy way to slow PacMan down
	if ((LoopCount2 % 8) == 0)
	{
		MovePacMan();
		CollisionCheck();
	...
	//When Pacman is not moving, perform the Player Modelling functionality
	//We will subdivide this code between cycles using the LoopCount static
	else if(LoopCount2 == 1)
	{
		...
		char indexStr[199];
		char numBin[3];
		strcpy(indexStr, "PacXY: ");
		strcat(indexStr, itoa(xPosIndex, numBin, 10));
		strcat(indexStr, ", ");
		strcat(indexStr, itoa(yPosIndex, numBin, 10));
		strcat(indexStr, ";\t\t");
		
		//Obtain x and y pos values from the Ghost1 sprite pointer
		xPosInitial = g_ShadowSpritePos.x;
		yPosInitial = g_ShadowSpritePos.y;

		//Adjust position values to obtain index values
		xPosIndex = (int)((xPosInitial - GRIDTLX)/SPRITE_WIDTH);
		yPosIndex = (int)((yPosInitial - GRIDTLY)/SPRITE_HEIGHT);

		strcat(indexStr, "ShadowXY: ");
		strcat(indexStr, itoa(xPosIndex, numBin, 10));
		strcat(indexStr, ", ");
		strcat(indexStr, itoa(yPosIndex, numBin, 10));
		strcat(indexStr, ";\t\t");

		//Obtain x and y pos values from the Ghost2 sprite pointer
		xPosInitial = g_SpeedySpritePos.x;
		yPosInitial = g_SpeedySpritePos.y;

		//Adjust position values to obtain index values
		xPosIndex = (int)((xPosInitial - GRIDTLX)/SPRITE_WIDTH);
		yPosIndex = (int)((yPosInitial - GRIDTLY)/SPRITE_HEIGHT);

		strcat(indexStr, "SpeedyXY: ");
		strcat(indexStr, itoa(xPosIndex, numBin, 10));
		strcat(indexStr, ", ");
		strcat(indexStr, itoa(yPosIndex, numBin, 10));
		strcat(indexStr, ";\t\t");

		//Obtain x and y pos values from the Ghost3 sprite pointer
		xPosInitial = g_PokeySpritePos.x;
		yPosInitial = g_PokeySpritePos.y;

		//Adjust position values to obtain index values
		xPosIndex = (int)((xPosInitial - GRIDTLX)/SPRITE_WIDTH);
		yPosIndex = (int)((yPosInitial - GRIDTLY)/SPRITE_HEIGHT);

		strcat(indexStr, "PokeyXY: ");
		strcat(indexStr, itoa(xPosIndex, numBin, 10));
		strcat(indexStr, ", ");
		strcat(indexStr, itoa(yPosIndex, numBin, 10));
		strcat(indexStr, ";\t\t");

		//Obtain x and y pos values from the Ghost4 sprite pointer
		xPosInitial = g_BashfulSpritePos.x;
		yPosInitial = g_BashfulSpritePos.y;

		//Adjust position values to obtain index values
		xPosIndex = (int)((xPosInitial - GRIDTLX)/SPRITE_WIDTH);
		yPosIndex = (int)((yPosInitial - GRIDTLY)/SPRITE_HEIGHT);

		strcat(indexStr, "BashfulXY: ");
		strcat(indexStr, itoa(xPosIndex, numBin, 10));
		strcat(indexStr, ", ");
		strcat(indexStr, itoa(yPosIndex, numBin, 10));
		strcat(indexStr, ";\n");
	...
	//Animate ghosts depending on current game loop count 
	//Note: This is a clumsy way to slow the ghosts down
	if ((LoopCount3 % 12) == 0)
	{
...
void MovePacMan()
{
	...
	//If a legal move occur, check its consequences
	if (bCheckMove)
	{
...
void MoveGhosts()
{
	int choice = 0; // north = 0, south = 1, east = 2, west = 3;
	
	//"Shadow" Ghost Sprite
	// chose a random number between 0 and 3 to decide on a random direction to move
	choice =  int( 4*( (double)rand() / (double)(RAND_MAX+1) )); // weird double casting required
	SetSpritePosition(&g_ShadowSpritePos, choice);

	//"Speedy" Ghost Sprite
	choice =  int( 4*( (double)rand() / (double)(RAND_MAX+1) ));
	SetSpritePosition(&g_SpeedySpritePos, choice);

	//"Pokey" Ghost Sprite
	choice =  int( 4*( (double)rand() / (double)(RAND_MAX+1) ));
	SetSpritePosition(&g_PokeySpritePos, choice);

	//"Bashful" Ghost Sprite
	choice =  int( 4*( (double)rand() / (double)(RAND_MAX+1) )); 
	SetSpritePosition(&g_BashfulSpritePos, choice);
...
void SetSpritePosition(LPD3DXVECTOR3 pSpritePos, int nChoice)
{
	...
	// note that if the move selected by the random number is not valis then there is no movement - possibly not a good technique
	switch (nChoice)
	{
	case 0:
	...
	case 2:
		{
			if (PacMap[xPosIndex+1][yPosIndex] > 1)
				xPosFinal++;
	...
	SetPositionVector(pSpritePos, (GRIDTLX + (xPosFinal * SPRITE_WIDTH)), 
                                  (GRIDTLY + (yPosFinal * SPRITE_HEIGHT))); 
