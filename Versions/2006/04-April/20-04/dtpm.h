	...
	for(int g1 = 0; g1 < 4; g1++)
	{
		...
		case 0:
			if( PacMap[ghostIndices[0][0]][ghostIndices[0][1] - 1 ] == 1 )
				fProbs[g1][0][0][0] = 0;
				//Also add 1/3 to all other states for this ghost, or 1/2 if this is second blocked direction
				//addToAllProbsForThisGhost()
				//	if(thereIsOneBlockedDir) fraction = 4/3; else fraction = 9/8;
				//	for(whateverIterationIsNecessary) fProbs[g1][][][] *= fraction;
			break;
		case 1:
			if( PacMap[ghostIndices[0][0]][ghostIndices[0][1] + 1 ] == 1 )
				fProbs[g1][0][0][0] = 0;
