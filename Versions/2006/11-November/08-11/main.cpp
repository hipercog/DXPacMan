...
INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, INT iCmdShow)
{
	...
	long lastSec = time(0);
	int frameTimer = 0, lastSecFrames = 60, pacFrameTimer = 0, gstFrameTimer = 0;
	deque <int> prevFrames;
	double pacTik = 5.0, gstTik = 7.5;
...
//	gstTik = pacTik*1.5;
	...
	while( TRUE )
	{
		if(lastSec != time(0))
		{
			prevFrames.push_front(frameTimer);
			lastSec = time(0);
			lastSecFrames = frameTimer;
			pacTik = lastSecFrames/12.0; //avgOver(prevFrames)/12.0;
			gstTik = pacTik*1.5;
			cout << "Frames: " << frameTimer << "; Loops: " << pacloop << ", " << gstloop << "; Tiks: " << pacTik << ", " << gstTik << endl;
			frameTimer = 0;
			pacloop = gstloop = 0;
		}
...
int GameShutdown()
{
	//Flush the text output & clear up dtpm.h globals
	deInit();
...
void MovePacMan()
{
	...
	if (keyboard_state[DIK_UP]) //"Up" arrow has been pressed    
	{
		...
		if (PacMap[xPosIndex][yPosIndex-1] > 2) 
		{
			...
			if(g_iPredHunt == 0)
				CorrectHunt++;
			else if(g_iPredHunt > -1)
				InCorrectHunt++;
			if(g_iPredCherry == 0)
				CorrectCherry++;
			else if(g_iPredCherry > -1)
				InCorrectCherry++;
			if(g_iPred2Pill == 0)
				Correct2Pill++;
			else if(g_iPred2Pill > -1)
				InCorrect2Pill++;
			if(g_iPredRun == 0)
				CorrectRun++;
			else if(g_iPredRun > -1)
				InCorrectRun++;
			if(g_iPredDots == 0)
				CorrectDots++;
			else if(g_iPredDots > -1)
				InCorrectDots++;
//			print("\n****  MOVED NORTH  ****\n");
		}
	}
