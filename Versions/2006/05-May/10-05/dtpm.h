...
void probabilityOfGhostState(unsigned int recurseLimit, int *baseGhostX, int *baseGhostY);//int xPacLoc, int yPacLoc);
double planOfActions(int timeHorizon);
void availableActions(int xPac, int yPac);
double utilityOfObjective(int src_x, int src_y, int trg_x, int trg_y);
...
void availableActions(int xPac, int yPac)
{
	...
	if(ShadowMap[xPac][yPac-1] > 2)
		movUtil[0] = utilityOfObjective(xPac, yPac - 1, x1, y1);
	if(ShadowMap[xPac][yPac+1] > 2)
		movUtil[1] = utilityOfObjective(xPac, yPac + 1, x1, y1);
	if(ShadowMap[xPac+1][yPac] > 2)
		movUtil[2] = utilityOfObjective(xPac + 1, yPac, x1, y1);
	if(ShadowMap[xPac-1][yPac] > 2)
        movUtil[3] = utilityOfObjective(xPac - 1, yPac, x1, y1);
...
sprintf(out, "%G", utilityOfObjective(xPac, yPac, x1, y1)); print("\nUtil@PacmanXY:"); print(out);
...
/*vector<double>*/ double utilityOfObjective(int src_x, int src_y, int trg_x, int trg_y)
{
	double util[4] = {0.0};

	if(a_Star(src_x, src_y - 1, trg_x, trg_y) != 99)
	{
//			itoa(aPathX.size(), out, 10); print("\naPathX length: "); print(out);
//			itoa(aPathY.size(), out, 10); print("\naPathY length: "); print(out);
		for(int i = 0; i < aPathX.size(); i++)
		{
	...
	if(a_Star(src_x, src_y + 1, trg_x, trg_y) != 99)
	{
		...
		util[1] += 10 - aPathX.size();
	}
	if(a_Star(src_x + 1, src_y, trg_x, trg_y) != 99)
	{
		for(int i = 0; i < aPathX.size(); i++)
		{
			if(ShadowMap[aPathX[i]][aPathY[i]] == 4)
	...
	if(a_Star(src_x - 1, src_y, trg_x, trg_y) != 99)
	{
		...
		util[3] += 10 - aPathX.size();
	}
	double tmpArr[4] = {util[0], util[1], util[2], util[3]};

	print("\nUnSorted utils: ");
sprintf(out, "%G", tmpArr[0]); print(out); print(", ");
sprintf(out, "%G", tmpArr[1]); print(out); print(", ");
sprintf(out, "%G", tmpArr[2]); print(out); print(", ");
sprintf(out, "%G", tmpArr[3]); print(out); print("\n");
...
//	vector<double>retval(2) = {dir, tmpArr[3]};
//	return retval;
...
/**/
	...
	if(!CherryEaten)
	{
		...
		int y1 = yIndex;
		getSpritePos(g_PacmanSpritePos);
		util = utilityOfObjective(xIndex, yIndex, x1, y1);
	}
