...
//	itoa(myHeuristicDist(1, 18, 9, 9), out, 10); print("My H Dist: "); print(out);
//	itoa(manhattanDist(1, 18, 9, 9), out, 10); print("mhtn Dist: "); print(out);
...
void availableActions(int xPac, int yPac)
{
	...
	//Lets posit and test some heuristics - start with most urgent, i.e. what would a good player prioritise? 
	//We can only look at one at a time, so this will be an if/else structure - could we use a more intricate decision logic?
	if(!CherryEaten)	//We need to eat that cherry!
	{
		getSpritePos(g_CherrySpritePos);
		x1 = xIndex;
		y1 = yIndex;
	}else if(localGhosts(xPac, yPac, 6) > 2)	// There are Ghosts close by - either get a PowerPill or run away.
	{
		int xPill = 18, yPill = 15;
		int pill0 = myHeuristicDist(xPac, yPac, 1, 3);
		int pill1 = myHeuristicDist(xPac, yPac, 18, 3);
		int pill2 = myHeuristicDist(xPac, yPac, 1, 15);
		int pill3 = myHeuristicDist(xPac, yPac, 18, 15);
		if(pill0 < pill1)
		{
			xPill = 1;
			if(pill0 < pill2)
			...
			if(pill1 < pill3)
				yPill = 3;
		if(a_Star(xPac, yPac, xPill, yPill) < 6)
		{
		}
	}
	...
	if(myHeuristicDist(xPac, yPac, x1, y1) < 6)
	{
		if(ShadowMap[xPac][yPac-1] > 2)
			movUtil[0] = utilityPointToPoint(xPac, yPac - 1, x1, y1);
		if(ShadowMap[xPac][yPac+1] > 2)
			movUtil[1] = utilityPointToPoint(xPac, yPac + 1, x1, y1);
		if(ShadowMap[xPac+1][yPac] > 2)
			movUtil[2] = utilityPointToPoint(xPac + 1, yPac, x1, y1);
		if(ShadowMap[xPac-1][yPac] > 2)
			movUtil[3] = utilityPointToPoint(xPac - 1, yPac, x1, y1);
	}
