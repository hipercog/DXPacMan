...
char spawn(TreeIterator<GhostMoves> iter);
void spawn(Tree<GhostMoves>* tree);
...
/*
	while( count < recurseLimit )
	{
		...
		if( 0 == 0)
		{
		}else //shunt the fail-result up to the call to recurse
			return false;
		count++;
	}
 ...
 * Argument: Iterator that points to the current node we want to grow - could use the global iterator maybe
 */
void spawn(Tree<GhostMoves>* tree)
{
	...
	// Global Tree gets new nodes just appended to local iterator - could use global iter instead of argument?
	// tree_GstMv
}
