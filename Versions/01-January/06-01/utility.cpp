...
float utility( deque <PacState> States )
{
	...
	for( int i = S; i > max( 0, S-lookahead ); i-- )
	{
		...
		{
/**/
			utils.push_back( util_A4_HuntAfterPill( States ) );
			feats.push_back( ",HuntAfter:," );
/**/
			utils.push_back( util_A6_Chase¬Dots( States ) );
			feats.push_back( ",Chase¬Dots:," );
/**/
			utils.push_back( util_C4_CaughtAfterHunt( States ) );
			feats.push_back( ",CaughtAfterHunt:," );
/**/
			utils.push_back( 15 - util_C2_AvgDstGstCtrd( States ) );
	...
	{
/**/
		utils.push_back( util_C1_ThreatPrcptn( States ) );
		feats.push_back( ",ThreatPrcptn:," );
/**/
		utils.push_back( util_C3_CloseCalls( States ) );
...
/**/
		utils.push_back( util_C2_AvgDstGstCtrd( States ) );
		feats.push_back( ",AvgDist:," );
	}

	utils.push_back( States[s-1].Points - actual_States[S].Points );
...
/**/
	...
	{
		utils.push_back( util_Agg2_ChryBoolSum( States ) );
		feats.push_back( ",CountUntilCherryEaten:," );
	}
/**/
	utils.push_back( util_S2_LivesChng( States ) );
