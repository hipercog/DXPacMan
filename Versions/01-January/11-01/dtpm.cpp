...
float matches = 0.0, accuracy = 1.0, tendency = 0.01;
 ...
 */
void postDTPM( unsigned int look_AHead )
{
	lookahead = look_AHead;
	deque<int> csv = readFromCSVFile( "stateData/alanFullGame-1.csv" );
/**/
	int tempMap[20][20], xGst[NUM_GS], yGst[NUM_GS];
	int addend = 2 + (NUM_GS * 2);
	int i = 0, predir = -1, compdir = -1, temp, count;
	...
	while( i < csv.size() )
	{
		...
		if( actual_States[ actual_States.size()-2 ].Lives > real_State.Lives )
			stateCount--;
/**/
...
/**/
		...
		if( base_nm.prob == 2 )
		{
			...
			if( moved[ real_State.MovedDir ] > 6 )
			{
/** /
				for( int o = 0; o < 4; o++ )
				{
					if( base_nm.xyMove[0][o] != 0 && o != real_State.MovedDir )
						predir = o;
				}
/**/
...
/* Also prevent backtracking from junctions: seems to have no effect - still not ruled out until further testing. * /
		}else if( base_nm.prob == 3 || base_nm.prob == 4 )
		{
			if( moved[ real_State.MovedDir ] > 4 )
			{
				while( predir == real_State.MovedDir && branchUtils.size() > 0 )
				{
					branchUtils.erase( branchUtils.begin() + index );
...
/**/
		...
		if( matches / (float)(stateCount + real_State.State) > accuracy )
		{
			learn( (stateCount + real_State.State)%9, tendency );
		}else{
