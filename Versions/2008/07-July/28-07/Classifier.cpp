...
float ClassifyConqueror_Level( deque<PacState> arg )
{
	...
	usage.assign( 35, 0 );	//Prepare holder for feature usage stats - #33 is Conq rule firing, #34 is Not_Conq rule firing
	 ...
	 */
	if( Lives_SDev > 1.5 ){ conq -= 0.235264706; usage[15]++; usage[34]++; }

	if( D2_Osclltng > 122 ){ conq -= 0.2133; usage[14]++; usage[34]++; }

	if( A4_HuntAfterPill <= 1 ){ conq -= 0.213213115; usage[2]++; usage[34]++; }

	if( Points_Max <= 2000 ){ conq += 0.161826824; usage[24]++; usage[33]++; }

	if( A4_HuntAfterPill > 2 ){ conq += 0.147540984; usage[2]++; usage[33]++; }

	if( S1c_AvgCycles > 5730 ){ conq -= 0.147540984; usage[28]++; usage[34]++; }

	if( ChryBool_Sum <= 75 ){ conq -= 0.1333; usage[11]++; usage[34]++; }

	if( C5_MvsWoutGain > 100 ){ conq -= 0.1067; usage[8]++; usage[34]++; }

	if( P7_PutOffGstHse > 0 ){ conq += 0.106509836; usage[23]++; usage[33]++; }
	 ...
	 * /
	if( A4_HuntAfterPill > 0.460 && C4_CaughtAfterHunt <= 0.557 ){ conq += 0.226733333; usage[2]++; usage[7]++; usage[33]++; }

	if( C5_MvsWoutGain <= 0.413 && S2a_LivesGained > 0.620 ){ conq += 0.213196721; usage[8]++; usage[29]++; usage[33]++; }

	if( A4_HuntAfterPill > 0.167 && S1b_AvgStates <= 0.636 ){ conq += 0.191264706; usage[2]++; usage[27]++; usage[33]++; }

	if( A4_HuntAfterPill > 0.563 && S1c_AvgCycles <= 0.589 ){ conq += 0.161764706; usage[2]++; usage[28]++; usage[33]++; }

	if( A4_HuntAfterPill > 0.212 && ChryBool_Sum <= 0.138 ){ conq -= 0.147540984; usage[2]++; usage[11]++; usage[34]++; }

	if( Points_Max <= 0.448 && P4b_HuntGst2_AvgSpd <= 0.017 ){ conq += 0.132352941; usage[24]++; usage[22]++; usage[33]++; }

	if( D2_Osclltng <= 0.678 && R1_AvgTimeToEatChrry <= 0.145 ){ conq -= 0.126575738; usage[14]++; usage[25]++; usage[34]++; }

	if( ChryBool_Sum > 0.137 && C3_CloseCalls <= 0.116 ){ conq += 0.12; usage[11]++; usage[25]++; usage[33]++; }

	if( ChryBool_Sum > 0.136 && A6_Chase¬Dots > 0.450 ){ conq -= 0.102941176; usage[11]++; usage[3]++; usage[34]++; }
	 ...
	 * /
	if( C5_MvsWoutGain > 0.413 && P4b_HuntGst2_AvgSpd <= 0.504 && P7_PutOffGstHse <= 0.469 ){ conq -= 0.393491803; usage[8]++; usage[22]++; usage[23]++; }

	if( S2a_LivesGained > 0.620 && S1c_AvgCycles <= 0.589 && Lives_SDev <= 0.659 ){ conq += 0.327836066; usage[29]++; usage[28]++; usage[15]++; usage[33]++; }

	if( A4_HuntAfterPill <= 0.563 && S2a_LivesGained <= 0.627 && Lives_SDev <= 0.578 ){ conq -= 0.327737705; usage[2]++; usage[29]++; usage[15]++; }

	if( S2a_LivesGained > 0.620 && C4_CaughtAfterHunt <= 0.557 && S1c_AvgCycles <= 0.680 ){ conq += 0.323647059; usage[29]++; usage[7]++; usage[28]++; usage[33]++; }

	if( C5_MvsWoutGain > 0.305 && C3_CloseCalls > 0.103 && C3_CloseCalls <= 0.206 ){ conq -= 0.311442623; usage[8]++; usage[6]++; usage[6]++; usage[34]++; }

	if( S1c_AvgCycles <= 0.458 && A4_HuntAfterPill > 0.181 && S2a_LivesGained > 0.429 ){ conq += 0.294117647; usage[28]++; usage[2]++; usage[29]++; usage[33]++; }

	if( C6_PntsOvrLives <= 0.506 && Lives_SDev <= 0.603 && S2a_LivesGained > 0.620 ){ conq += 0.279529412; usage[9]++; usage[15]++; usage[29]++; usage[33]++; }

	if( ChryBool_Sum > 0.137 && S2a_LivesGained > 0.620 && Lives_SDev <= 0.602 ){ conq += 0.2667; usage[11]++; usage[29]++; usage[15]++; usage[33]++; }

	if( Points_Max > 0.448 && P3_CntDtsB4Pll1 > 0.191 && P4a_HuntGst1_AvgSpd <= 0.676 ){ conq -= 0.220705882; usage[24]++; usage[20]++; usage[21]++; usage[34]++; }
	
	if( R2_AvgChrryPerLvl > 0.333 && P7_PutOffGstHse <= 0.438 && P7_PutOffGstHse > 0.292 ){ conq -= 0.219350147; usage[26]++; usage[23]++; usage[23]++; usage[34]++; }

	if( A4_HuntAfterPill <= 0.563 && R2_AvgChrryPerLvl <= 0.802 && A3_GhostKills > 0.378 ){ conq -= 0.213114754; usage[2]++; usage[26]++; usage[1]++; usage[34]++; }

	if( A4_HuntAfterPill > 0.173 && S5_GhostHunts > 0.941 && CycleCountPerState_SDev > 0.229 ){ conq -= 0.2; usage[2]++; usage[32]++; usage[12]++; usage[34]++; }

	if( A4_HuntAfterPill > 0.136 && S1b_AvgStates <= 0.639 && C6_PntsOvrLives <= 0.510 ){ conq += 0.176455882; usage[2]++; usage[27]++; usage[9]++; usage[33]++; }

	if( S1b_AvgStates <= 0.824 && C4_CaughtAfterHunt > 0.356 && S5_GhostHunts > 0.911 ){ conq -= 0.174110588; usage[27]++; usage[7]++; usage[32]++; usage[34]++; }
	
	if( R2_AvgChrryPerLvl > 0.333 && P7_PutOffGstHse > 0.438 && Lives_SDev <= 0.597 ){ conq += 0.163206441; usage[26]++; usage[23]++; usage[15]++; usage[33]++; }

	if( A4_HuntAfterPill <= 0.563 && S2a_LivesGained > 0.627 && Lives_SDev > 0.609 ){ conq -= 0.131163934; usage[2]++; usage[29]++; usage[15]++; usage[34]++; }

	if( ChryBool_Sum > 0.137 && C3_CloseCalls > 0.116 && A4_HuntAfterPill > 0.567 ){ conq += 0.119973333; usage[11]++; usage[6]++; usage[2]++; usage[33]++; }

	if( S2a_LivesGained <= 0.620 && A4_HuntAfterPill > 0.257 && A6_Chase¬Dots <= 0.124 ){ conq += 0.117647059; usage[29]++; usage[2]++; usage[3]++; usage[33]++; }
	...
	{
		conq -= 0.161764706; 
		usage[19]++; usage[4]++; usage[26]++; usage[18]++; usage[17]++; usage[21]++; usage[26]++; usage[3]++; usage[34]++;
	}
