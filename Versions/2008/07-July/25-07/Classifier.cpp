...
float P1b_2GstsLrd2Pll_Avg = 0.0;
float P1c_GstsEtnOnPll_Avg = 0.0;
float P1d_PacEtnB4GetPll_Avg = 0.0;
...
/**/
	...
	deque <float> caution2 = Caution2_AvgDstPacGst( arg );
	/**/
...
CycleCountPerState_SDev = Aggregate3_CycleCountPerStateSDev( arg );	// .10	6.9 - 57.3	|				| 0.12 - 1
	...
	P4b_HuntGst2_AvgSpd = plan4[2] / max(1,A3_GhostKills-4);		// .21	0 - 2307	|				| 0 - 1

	Points_Max = arg[s-1].Points;
/**/
	A1_AtkGstHse = Aggression1_AtkGstHse( arg );					// .22	0 - 35		| 0 - 3.2		| 0 - 1

	C2b_AvgDstPacAtkCntrd = caution2[3];							// .23	7.02 - 14.09|				| 0.498 - 1

	C6_PntsOvrLives = Caution6_PntsOvrLives( arg );					// .24	142 - 3750	|				| 0.038 - 1

	D1_CyclesPerMove = Decisive1_CyclesPrMv( arg );					// .25	7.2 - 42.4	|				| 0.17 - 1

	P1b_2GstsLrd2Pll_Avg = plan1[2];								// .26	0 - 9		| 0 - 1			| 0 - 1

	P1c_GstsEtnOnPll_Avg = plan1[3];								// .27	0 - 37		| 0 - 3.6		| 0 - 1

	P1d_PacEtnB4GetPll_Avg = plan1[4];								// .28	0 - 1		| 0 - 0.333		| 0 - 1

	P3_CntDtsB4Pll1 = Planning3_CntDtsB4Pll1( arg );				// .29	6.3 - 97.6	| 0.9 - 22		| 0.041 - 1

	S5_GhostHunts = 4.0 - arg[s-1].Pills;							// .30	# hunts = # Pills used

P4a_HuntGst1_AvgSpd = plan4[0] / min(S5_GhostHunts, A3_GhostKills);	// .31	62 - 349	|				| 0.178 - 1

	S2b_LivesLost = simple2[1];										// .32	0 - 19		|				| 0 - 1
...
/**/
	...
	result.push_back( P1b_2GstsLrd2Pll_Avg );	feature.push_back( "P1b_2GstsLrd2Pll_Avg" );
	result.push_back( P1c_GstsEtnOnPll_Avg );	feature.push_back( "P1c_GstsEtnOnPll_Avg" );
	result.push_back( P1d_PacEtnB4GetPll_Avg );	feature.push_back( "P1d_PacEtnB4GetPll_Avg" );
