 ...
 */
	A1_AtkGstHse[0] = recordSum( A1_AtkGstHse );
	A3_GhostKills[0] = recordSum( A3_GhostKills );
	A4_HuntAfterPill[0] = recordSum( A4_HuntAfterPill );
	A6_Chase¬Dots[0] = recordSum( A6_Chase¬Dots );
	C1b_ThreatPerception_NumKilled[0] = recordSum( C1b_ThreatPerception_NumKilled );
	C2b_AvgDstPacAtkCntrd[0] = recordSum( C2b_AvgDstPacAtkCntrd );
	C3_CloseCalls[0] = recordSum( C3_CloseCalls );
	C4_CaughtAfterHunt[0] = recordSum( C4_CaughtAfterHunt );
	C5_MvsWoutGain[0] = recordSum( C5_MvsWoutGain );
	C6_PntsOvrLives[0] = recordSum( C6_PntsOvrLives );
	C7_KilledAtHouse[0] = recordSum( C7_KilledAtHouse );
	ChryBool_Sum[0] = recordSum( ChryBool_Sum );
	CycleCountPerState_SDev[0] = recordSum( CycleCountPerState_SDev );
	D1_CyclesPerMove[0] = recordSum( D1_CyclesPerMove );
	D2_Osclltng[0] = recordSum( D2_Osclltng );
	Lives_SDev[0] = recordSum( Lives_SDev );
	P1_LureToPill[0] = recordSum( P1_LureToPill );
	P1b_2GstsLrd2Pll[0] = recordSum( P1b_2GstsLrd2Pll );
	P1c_GstsEtnOnPll[0] = recordSum( P1c_GstsEtnOnPll );
	P1d_PacEtnB4GetPll[0] = recordSum( P1d_PacEtnB4GetPll );
	P3_CntDtsB4Pll1[0] = recordSum( P3_CntDtsB4Pll1 );
	P4a_HuntGst1_AvgSpd[0] = recordSum( P4a_HuntGst1_AvgSpd );
	P4b_HuntGst2_AvgSpd[0] = recordSum( P4b_HuntGst2_AvgSpd );
	P7_PutOffGstHse[0] = recordSum( P7_PutOffGstHse );
	R1_AvgTimeToEatChrry[0] = recordSum( R1_AvgTimeToEatChrry );
	R2_AvgChrryPerLvl[0] = recordSum( R2_AvgChrryPerLvl );
	S1b_AvgStates[0] = recordSum( S1b_AvgStates );
	S1c_AvgCycles[0] = recordSum( S1c_AvgCycles );
	S2a_LivesGained[0] = recordSum( S2a_LivesGained );
	S2b_LivesLost[0] = recordSum( S2b_LivesLost );
	S4_TeleportUse[0] = recordSum( S4_TeleportUse );
	S5_GhostHunts[0] = recordSum( S5_GhostHunts );
...
float ClassifyConqueror_Level()
{
	 ...
	 */
	if( Lives_SDev[l] > 1.5 ){ conq -= 0.235264706; usage[15]++; fired[0]--; }

	if( D2_Osclltng[l] > 122 ){ conq -= 0.2133; usage[14]++; fired[1]--; }

	if( A4_HuntAfterPill[l] <= 1 ){ conq -= 0.213213115; usage[2]++; fired[2]--; }

	if( Points_Max[l] <= 1895 ){ conq += 0.161826824; usage[24]++; fired[3]++; }

	if( A4_HuntAfterPill[l] > 2 ){ conq += 0.147540984; usage[2]++; fired[4]++; }

	if( S1c_AvgCycles[l] > 5500 ){ conq -= 0.147540984; usage[28]++; fired[5]--; }

	if( ChryBool_Sum[l] <= 24 ){ conq -= 0.1333; usage[11]++; fired[6]--; }

	if( C5_MvsWoutGain[l] > 150 ){ conq -= 0.1067; usage[8]++; fired[7]--; }

	if( P7_PutOffGstHse[l] == 1 ){ conq += 0.106509836; usage[23]++; fired[8]++; }
	 ...
	 */
	if( A4_HuntAfterPill[l] > 2 && C4_CaughtAfterHunt[l] <= 2 ){ conq += 0.226733333; usage[2]++; usage[7]++; fired[9]++; }

	if( C5_MvsWoutGain[l] <= 106 && S2a_LivesGained[l] == 1 ){ conq += 0.213196721; usage[8]++; usage[29]++; fired[10]++; }

	if( A4_HuntAfterPill[l] >= 1 && S1b_AvgStates[l] <= 304 ){ conq += 0.191264706; usage[2]++; usage[27]++; fired[11]++; }

	if( A4_HuntAfterPill[l] > 2 && S1c_AvgCycles[l] <= 5650 ){ conq += 0.161764706; usage[2]++; usage[28]++; fired[12]++; }

	if( A4_HuntAfterPill[l] > 1 && ChryBool_Sum[l] <= 23 ){ conq -= 0.147540984; usage[2]++; usage[11]++; fired[13]--; }

	if( Points_Max[l] <= 1950 && P4b_HuntGst2_AvgSpd[l] <= 38 ){ conq += 0.132352941; usage[24]++; usage[22]++; fired[14]++; }

	if( D2_Osclltng[l] <= 123 && R1_AvgTimeToEatChrry[l] <= 21 ){ conq -= 0.126575738; usage[14]++; usage[25]++; fired[15]--; }

	if( ChryBool_Sum[l] > 23 && C3_CloseCalls[l] <= 2 ){ conq += 0.12; usage[11]++; usage[25]++; fired[16]++; }

	if( ChryBool_Sum[l] > 23 && A6_Chase¬Dots[l] > 5 ){ conq -= 0.102941176; usage[11]++; usage[3]++; fired[17]--; }

	if( C5_MvsWoutGain[l] > 79 && C3_CloseCalls[l] == 1 ){ conq -= 0.311442623; usage[8]++; usage[6]++; usage[6]++; fired[18]--; }
	 ...
	 */
	if( C5_MvsWoutGain[l] > 106 && P4b_HuntGst2_AvgSpd[l] <= 1164 && P7_PutOffGstHse[l] == 0 ){ conq -= 0.393491803; usage[8]++; usage[22]++; usage[23]++; fired[19]--; }

	if( S2a_LivesGained[l] == 1 && S1c_AvgCycles[l] <= 5650 && Lives_SDev[l] <= 1.654 ){ conq += 0.327836066; usage[29]++; usage[28]++; usage[15]++; fired[20]++; }

	if( A4_HuntAfterPill[l] <= 2 && S2a_LivesGained[l] == 0 && Lives_SDev[l] <= 1.469 ){ conq -= 0.327737705; usage[2]++; usage[29]++; usage[15]++; fired[21]--; }

	if( S2a_LivesGained[l] == 1 && C4_CaughtAfterHunt[l] <= 2 && S1c_AvgCycles[l] <= 6530 ){ conq += 0.323647059; usage[29]++; usage[7]++; usage[28]++; fired[22]++; }

	if( S1c_AvgCycles[l] <= 4410 && A4_HuntAfterPill[l] > 1 && S2a_LivesGained[l] == 1 ){ conq += 0.294117647; usage[28]++; usage[2]++; usage[29]++; fired[23]++; }

	if( C6_PntsOvrLives[l] <= 1900 && Lives_SDev[l] <= 1.54 && S2a_LivesGained[l] == 1 ){ conq += 0.279529412; usage[9]++; usage[15]++; usage[29]++; fired[24]++; }

	if( ChryBool_Sum[l] > 23 && S2a_LivesGained[l] == 1 && Lives_SDev[l] <= 1.54 ){ conq += 0.2667; usage[11]++; usage[29]++; usage[15]++; fired[25]++; }

	if( Points_Max[l] > 1950 && P3_CntDtsB4Pll1[l] > 19 && P4a_HuntGst1_AvgSpd[l] <= 230 ){ conq -= 0.220705882; usage[24]++; usage[20]++; usage[21]++; fired[26]--; }
	...
	if( R2_AvgChrryPerLvl[0] > 0.333 && (P7_PutOffGstHse[0] / l-1) <= 0.438 && (P7_PutOffGstHse[0] / l-1) >= 0.292 ){ conq -= 0.219350147; usage[26]++; usage[23]++; usage[23]++; fired[27]--; }

	if( A4_HuntAfterPill[l] <= 2 && R2_AvgChrryPerLvl[l] == 0 && A3_GhostKills[l] > 0 ){ conq -= 0.213114754; usage[2]++; usage[26]++; usage[1]++; fired[28]--; }
	...
	if( GameOver && A4_HuntAfterPill[l] > 1 && S5_GhostHunts[l] > 3.75 && CycleCountPerState_SDev[l] > 13 ){ conq -= 0.2; usage[2]++; usage[32]++; usage[12]++; fired[29]--; }

	if( A4_HuntAfterPill[l] > 0 && S1b_AvgStates[l] <= 305 && C6_PntsOvrLives[l] <= 1900 ){ conq += 0.176455882; usage[2]++; usage[27]++; usage[9]++; fired[30]++; }

	if( S1b_AvgStates[l] <= 393 && C4_CaughtAfterHunt[l] > 2 && S5_GhostHunts[l] > 3.75 ){ conq -= 0.174110588; usage[27]++; usage[7]++; usage[32]++; fired[31]--; }

	if( R2_AvgChrryPerLvl[l] == 1 && P7_PutOffGstHse[l] == 1 && Lives_SDev[l] <= 1.52 ){ conq += 0.163206441; usage[26]++; usage[23]++; usage[15]++; fired[32]++; }

	if( A4_HuntAfterPill[l] <= 2 && S2a_LivesGained[l] == 1 && Lives_SDev[l] > 1.55 ){ conq -= 0.131163934; usage[2]++; usage[29]++; usage[15]++; fired[33]--; }

	if( ChryBool_Sum[l] > 23 && C3_CloseCalls[l] > 0 && A4_HuntAfterPill[l] > 2 ){ conq += 0.119973333; usage[11]++; usage[6]++; usage[2]++; fired[34]++; }

	if( S2a_LivesGained[l] == 0 && A4_HuntAfterPill[l] > 1 && A6_Chase¬Dots[l] <= 1 ){ conq += 0.117647059; usage[29]++; usage[2]++; usage[3]++; fired[35]++; }
