...
/**/
	...
	while( i < csv.size() )
	{
		...
		{
			Lvl++;
			if( Lvl == 1 || Lvl == 3 || Lvl == 6 || Lvl == 10 || Lvl == 15 )
			{
				gakProb++;
				pacProb--;
			}
...
/**/
		...
		sprintf_s( out, 99, "%d", predir );
		print( "\nPred:," ); print( out );
		sprintf_s( out, 99, "%d", real_State.MovedDir );
		print( ",Moved:," ); print( out );

		int match = (int)(predir == real_State.MovedDir);
		sprintf_s( out, 99, "%d", match );
		print( ",Match:," ); print( out );

		matches += (float)match;
		sprintf_s( out, 99, "%f", (float)(matches / (float)real_State.State) );
		print( ",%Correct:," ); print( out ); print( "\n" );
		
		predir = branchDirs[ highest( branchUtils ) ];
...
PacState updateState( PacState pS )
{
	...
	if( pS.PacAttack )
	{
		...
		for( int i = 0; i < NUM_GS; i++ )
		{
			...
			{
				pS.Lives--;
				//We have a global bool here, to flag that Pac death - then the future moves exploration can be severed for this branch
				PredictDeath = true;
			}
