...
{
	A1_AtkGstHse[0] = recAvgWindow( A1_AtkGstHse, w );
	A3_GhostKills[0] = recAvgWindow( A3_GhostKills, w );
	A4_HuntAfterPill[0] = recAvgWindow( A4_HuntAfterPill, w );
	A6_Chase¬Dots[0] = recAvgWindow( A6_Chase¬Dots, w );
	C1b_ThreatPerception_NumKilled[0] = recAvgWindow( C1b_ThreatPerception_NumKilled, w );
	C2b_AvgDstPacAtkCntrd[0] = recAvgWindow( C2b_AvgDstPacAtkCntrd, w );
	C3_CloseCalls[0] = recAvgWindow( C3_CloseCalls, w );
	C4_CaughtAfterHunt[0] = recAvgWindow( C4_CaughtAfterHunt, w );
	C5_MvsWoutGain[0] = recAvgWindow( C5_MvsWoutGain, w );
	C6_PntsOvrLives[0] = recAvgWindow( C6_PntsOvrLives, w );
	C7_KilledAtHouse[0] = recAvgWindow( C7_KilledAtHouse, w );
	ChryBool_Sum[0] = recAvgWindow( ChryBool_Sum, w );
	CycleCountPerState_SDev[0] = recAvgWindow( CycleCountPerState_SDev, w );
	D1_CyclesPerMove[0] = recAvgWindow( D1_CyclesPerMove, w );
	D2_Osclltng[0] = recAvgWindow( D2_Osclltng, w );
	Lives_SDev[0] = recAvgWindow( Lives_SDev, w );
	P1_LureToPill[0] = recAvgWindow( P1_LureToPill, w );
	P1b_2GstsLrd2Pll[0] = recAvgWindow( P1b_2GstsLrd2Pll, w );
	P1c_GstsEtnOnPll[0] = recAvgWindow( P1c_GstsEtnOnPll, w );
	P1d_PacEtnB4GetPll[0] = recAvgWindow( P1d_PacEtnB4GetPll, w );
	P3_CntDtsB4Pll1[0] = recAvgWindow( P3_CntDtsB4Pll1, w );
	P4a_HuntGst1_AvgSpd[0] = recAvgWindow( P4a_HuntGst1_AvgSpd, w );
	P4b_HuntGst2_AvgSpd[0] = recAvgWindow( P4b_HuntGst2_AvgSpd, w );
	P7_PutOffGstHse[0] = recAvgWindow( P7_PutOffGstHse, w );
	R1_AvgTimeToEatChrry[0] = recAvgWindow( R1_AvgTimeToEatChrry, w );
	R2_AvgChrryPerLvl[0] = recAvgWindow( R2_AvgChrryPerLvl, w );
	S1b_AvgStates[0] = recAvgWindow( S1b_AvgStates, w );
	S1c_AvgCycles[0] = recAvgWindow( S1c_AvgCycles, w );
	S2a_LivesGained[0] = recAvgWindow( S2a_LivesGained, w );
	S2b_LivesLost[0] = recAvgWindow( S2b_LivesLost, w );
	S4_TeleportUse[0] = recAvgWindow( S4_TeleportUse, w );
	S5_GhostHunts[0] = recAvgWindow( S5_GhostHunts, w );
}
...
/**/
	...
	logData += "Feature,Value,use_wdw,use_avg,use_nve\n";
