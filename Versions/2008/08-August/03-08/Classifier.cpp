...
float ClassifyConqueror_Level( deque<PacState> arg )
{
	...
	if( C5_MvsWoutGain > 79 && C3_CloseCalls == 1 ){ conq -= 0.311442623; usage[8]++; usage[6]++; usage[6]++; fired[22]--; }
	...
	if( A4_HuntAfterPill <= 2 && S2a_LivesGained == 0 && Lives_SDev <= 1.469 ){ conq -= 0.327737705; usage[2]++; usage[29]++; usage[15]++; fired[20]--; }

	if( S2a_LivesGained == 1 && C4_CaughtAfterHunt <= 2 && S1c_AvgCycles <= 6530 ){ conq += 0.323647059; usage[29]++; usage[7]++; usage[28]++; fired[21]++; }
...
//	if( R2_AvgChrryPerLvl == 1 && P7_PutOffGstHse == 0 && P7_PutOffGstHse == 1 ){ conq -= 0.219350147; usage[26]++; usage[23]++; usage[23]++; fired[27]--; }
	...
	if( S2a_LivesGained == 0 && A4_HuntAfterPill > 1 && A6_Chase¬Dots <= 1 ){ conq += 0.117647059; usage[29]++; usage[2]++; usage[3]++; fired[35]++; }
	...
	if( A4_HuntAfterPill > 1 && S5_GhostHunts <= 3.75 && A6_Chase¬Dots <= 5 && ChryBool_Sum > 25 )
	...
	if( A4_HuntAfterPill > 0 && A1_AtkGstHse <= 0 && A6_Chase¬Dots <= 5 && P1c_GstsEtnOnPll > 0 )
	 ...
	 * /
	if( P1d_PacEtnB4GetPll <= 0 && C1b_ThreatPerception_NumKilled <= 2 && R2_AvgChrryPerLvl > 0.333 && P1c_GstsEtnOnPll <= 2 && P1b_2GstsLrd2Pll <= 1 && P4a_HuntGst1_AvgSpd <= 224 && R2_AvgChrryPerLvl <= 0.750 && A6_Chase¬Dots > 1 )
	{
		conq -= 0.161764706; 
