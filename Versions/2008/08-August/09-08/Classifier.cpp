...
deque <float> A1_AtkGstHse;
deque <float> A3_GhostKills;
deque <float> A4_HuntAfterPill;
deque <float> A6_Chase¬Dots;
deque <float> C1b_ThreatPerception_NumKilled;
deque <float> C2b_AvgDstPacAtkCntrd;
deque <float> C3_CloseCalls;
deque <float> C4_CaughtAfterHunt;
deque <float> C5_MvsWoutGain;
deque <float> C6_PntsOvrLives; // Score per Life per Level
deque <float> C7_KilledAtHouse;
deque <float> ChryBool_Sum;
deque <float> CycleCountPerState_SDev;
deque <float> D1_CyclesPerMove;
deque <float> D2_Osclltng;
deque <float> Lives_SDev;
deque <float> P1_LureToPill;
deque <float> P1b_2GstsLrd2Pll;
deque <float> P1c_GstsEtnOnPll;
deque <float> P1d_PacEtnB4GetPll;
deque <float> P3_CntDtsB4Pll1;
deque <float> P4a_HuntGst1_AvgSpd;
deque <float> P4b_HuntGst2_AvgSpd;
deque <float> P7_PutOffGstHse;
deque <float> Points_Max;
deque <float> R1_AvgTimeToEatChrry;
deque <float> R2_AvgChrryPerLvl;
deque <float> S1b_AvgStates;
deque <float> S1c_AvgCycles;
deque <float> S2a_LivesGained;
deque <float> S2b_LivesLost;
deque <float> S4_TeleportUse;
deque <float> S5_GhostHunts;
...
/**/
	...
	C6_PntsOvrLives.push_back( arg[s-1].Points / simple2[0] + simple2[1] );		// 9	2	142 - 3750	|				| 0.038 - 1	|
	...
	Points_Max.push_back( arg[s-1].Points - Points_Max[l] );					// 24	3	3200-62910	| 1303 - 4194	| 0.311 - 1	|

	R1_AvgTimeToEatChrry.push_back( resource[0] );								// 25	2	0 - 142		|				| 0 - 1		|

	R2_AvgChrryPerLvl.push_back( resource[1] );									// 26	8	0 - 1		|				| 0 - 1		|	

	S1b_AvgStates.push_back( simple[0] );										// 27	4	220 - 478	|				| 0.459 - 1	|

	S1c_AvgCycles.push_back( simple[1] );										// 28	5	2174 - 9598	|				| 0.226 - 1	|
...
//	if( R2_AvgChrryPerLvl[l] == 1 && P7_PutOffGstHse[l] == 0 && P7_PutOffGstHse[l] == 1 ){ conq -= 0.219350147; usage[26]++; usage[23]++; usage[23]++; fired[27]--; }
	...
	if( A4_HuntAfterPill[l] > 1 && S5_GhostHunts[l] > 3.75 && CycleCountPerState_SDev[l] > 13 ){ conq -= 0.2; usage[2]++; usage[32]++; usage[12]++; fired[29]--; }
