...
INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, INT iCmdShow)
{
	...
	getPlayerData();
	randCherryInit = 500;//rand()/11;
//itoa(randCherryInit, out, 10); print("Random cherry init: "); print(out);


	long frameTimer = 0, lastSec = time(0), lastSecFrames = 0;
	...
	while( TRUE )
	{
		...
		else
		{
			...
			/** /
			if(lastSecFrames%frameTimer == 0)
			PacLoop();
			/**/
			//GhostLoop();	//Different loop speed makes Ghosts slower
		}
