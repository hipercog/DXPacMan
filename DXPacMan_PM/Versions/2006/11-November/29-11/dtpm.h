...
double probabilityOfPac();
double probabilityOfGhost(unsigned int recurseLimit, int *baseGhostX, int *baseGhostY, int trg_x, int trg_y, bool targeting);
...
double probability()
{
	//Prob of the Pacman action and all the Ghosts' actions in response to Pacman
	return probabilityOfPac() + probabilityOfGhost(0, ghostIndicesX, ghostIndicesY, 0, 0, false);
}
...
//print("\nLogging ghost probOfState measure @ "); itoa(i, out, 10); print(out);
					util -= probabilityOfGhost(i, ghostIndicesX, ghostIndicesY, aPathX[i], aPathY[i], true);
					g = 4;
				}
				g++;
			}
...
/**
 * Probability of Pacman's move. This depends on the player, and so we can be either totally simple or throw in every possible heuristic.
 */
double probabilityOfPac()
{
	int openDirs = 0;
 ...
 * 
 */
double probabilityOfGhost(unsigned int recurseLimit, int *baseGhostX, int *baseGhostY, int trg_x, int trg_y, bool targeting/*[Ghost 1-4][x-y]*/)
{
...
/**/
	...
	print("-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------");
/**/
	//Recurses and finds the probability at the next step for each valid ghost move
	count = -1;
	for(int p1 = 0; p1 < 4; p1++)
	{
		for(int p2 = 0; p2 < 4; p2++)
...
/**/						if(targeting && (GhostMove[g2][0][count] == trg_x && GhostMove[g2][1][count] == trg_y))
						...
						if(recurseLimit > 0 && targeting)
						{
							...
							sttSpc += probabilityOfGhost(--recurseLimit, ghostBranchX, ghostBranchY, trg_x, trg_y, targeting);
						}
					}
				}
			}
