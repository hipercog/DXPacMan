...
print("\nHUNTING GHOSTIES\n");
		...
		int dst[4];
		for(int i = 0; i < 4; i++)
			dst[0] = myHeuristicDist(xPac, yPac, ghostIndicesX[i], ghostIndicesY[i]);
...
print("\nCHASING CHERRY\n");
	...
	}else if(localGhosts(xPac, yPac, 4) > 0)	// There are Ghosts close by - either get a PowerPill or run away.
	{
...
print("\nDOT COLLECTOR\n");
		...
		if(ShadowMap[xPac][yPac - 1] > 2)
			initVal[0] = movUtil[0] = utilityPointToPoint(xPac, yPac, xPac, yPac - 4);
		if(ShadowMap[xPac][yPac + 1] > 2)
			initVal[1] = movUtil[1] = utilityPointToPoint(xPac, yPac, xPac, yPac + 4);
		if(ShadowMap[xPac + 1][yPac] > 2)
			initVal[2] = movUtil[2] = utilityPointToPoint(xPac, yPac, xPac + 4, yPac);
		if(ShadowMap[xPac - 1][yPac] > 2)
			initVal[3] = movUtil[3] = utilityPointToPoint(xPac, yPac, xPac - 4, yPac);
		movUtil[4] = highest(initVal, 5);
	}
