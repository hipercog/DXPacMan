...
double actions( int depth, int lookahead );
void updateState();
 ...
 */
void forceDTPM(int lookahead)
{
	int temptime = 0;
	#ifdef _DEBUG
		cout << "Init data driven D.T. Pacman equation" << endl;
	...
	tree_GstMv->Destroy();
		...
		cout << "Ended D.T. Pacman equation. Time elapsed: " << temptime << endl;
	#endif
}
...
/**/
	...
	while( i < csv.size() )
	{
		int xGst[4] = { csv[i+3], csv[i+4], csv[i+5], csv[i+6] };
		int yGst[4] = { csv[i+7], csv[i+8], csv[i+9], csv[i+10] };
		int tempMap[20][20];
		int count = 0;
		for( int x = 0; x < 20; x++ )
			for( int y = 0; y < 20; y++ )
			{
				tempMap[x][y] = csv[i+17+count];
				count++;
			}
		S = PacState(csv[i+1], csv[i+2], csv[i+12], csv[i+14], csv[i+13], csv[i+11], xGst, yGst, csv[i+15], csv[i+16], tempMap );
	
		buildTreeOfGhostMoves( lookahead, -1, -1, S );
		iter_GstMv.Root();
		sprintf_s( out, 99, "%f", actions( lookahead, lookahead ) );
		print("Prediction: "); println(out);
		tree_GstMv->Destroy();
