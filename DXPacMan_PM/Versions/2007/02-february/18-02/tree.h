...
// ----------------------------------------------------------------
template <class DataType>
bool Preorder( Tree<DataType>* p_node, int (*p_process)(Tree<DataType>*), int depth )
{
	int check_depth = 0;
...
//print("\nDEPTH: "); sprintf_s(out, 99, "%d", check_depth); println(out);
	...
	else if( check_depth < depth )
	{
		...
		for( itr.Start(); itr.Valid(); itr.Forth() )
			Preorder( itr.Item(), p_process, depth );
	}
	return true;
}
...
// ----------------------------------------------------------------
template <class DataType>
double Postorder( Tree<DataType>* p_node, double (*p_process)(Tree<DataType>*), int depth )
{
	double retval = 0.0;
	int check_depth = 0;
    ...
    DListIterator<Tree<DataType>*> itr = p_node->m_children.GetIterator();
    for( itr.Start(); itr.Valid(); itr.Forth() )
        retval += Postorder( itr.Item(), p_process, depth );
