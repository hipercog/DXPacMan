...
void runclient(char *ip, const char *file, const char *out);
WComm w;
...
/**/	news += "\n";
	...
	if(local)
	{
		string temp = "C:/Documents and Settings/";
		temp += gameDir;
		if( fopen_s( &stateFile, temp.c_str(), "wt" ) != 0 )
	...
	}else
	{
		...
		runclient("127.0.0.1", gameDir.c_str(), news.c_str());
	}
}
 ...
 */
void runclient(char *ip, const char *file, const char *data)
{
	char rec[32] = "";
	...
	//Send file name
	w.sendData("NameSend");	w.recvData(rec,32);
	w.sendData(file);		w.recvData(rec,32);

	// Sending File
	w.sendData("DataSend");	w.recvData(rec,32);
