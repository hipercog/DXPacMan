...
int	PACMANSPRITE_START_POS_Y = 13;		//Start position y offset for PacMan sprite
int	CHERRYSPRITE_START_POS_Y = 11;		//Start position y offset for the cherry sprite

//Networking functions & fields
void runclient(char *ip, const char *out);
WComm w;
...
/**/	news += "\n";
	...
	if(local)
	{
		// Open for write (will fail if file does not exist?)
		string name = "C:\\Documents and Settings\\Ben\\My Documents\\PhD\\PacmanTypeTests\\stateData-";
		sprintf_s(out, 99, "%d", Level);
		name += out;
		name += ".csv";
		if( fopen_s( &stateFile, name.c_str(), "wt" ) != 0 )
			cout << "The file " << name.c_str() << "was not opened." << endl;
		//Put the latest gane state data onto the output file stream
		fputs(news.c_str(), stateFile);
	}else
		//If sending the data to a remote machine - IP for my Jumpline host: 66.84.48.128
		runclient("127.0.0.1", news.c_str());
}
...
void runclient(char *ip, const char *out)
{
	...
	w.sendData("DataSend");	
	w.recvData(rec,32);
	cout << "Instruction sent, ack'ed\n";

	w.sendData(out); w.recvData(rec,32);
	cout << "Data Sent!\n";
