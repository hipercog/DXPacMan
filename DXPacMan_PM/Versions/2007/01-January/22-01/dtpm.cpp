...
//	logPos(g_PacmanSpritePos, "PacmanXY: ", ";");
	...
	cout << "Utility is: " << utility(NULL) << endl;
	...
	for(int i = 0; i < 20; i++)
		for(int j = 0; j < 20; j++)
		{
			if(PacMap[i][j] == 4)
			{
				...
				util = (double)localDots(i, j, 1);
				if(util > 1)
					util += 0.33 * (localDots(i, j, 2) - util);
				//util = NumDots * 5; ??
			}
		}
	return util;
...
double utilityPills(int xPac, int yPac, int *xGhost, int *yGhost /*[Ghost 1-4][x-y]*/, int risk)
{
	...
	//Start with closest ghost to Pacman. Then next closest, etc.
	for(int i = 0; i < 4; i++)
	{
		mDist[i] = manhattanDist(xPac, yPac, xGhost[i], yGhost[i]);
	}
	for(int i = 0; i < 4; i++)
	{
		gst = lowest(mDist, 4);
		mDist[gst] = 99;
		util += (2^gst) * 100 / a_Star(xPac, yPac, xGhost[gst], yGhost[gst]);
		xPac = aPathX[aPathX.size()-1];
		yPac = aPathY[aPathY.size()-1];
	}
