...
/**/
	...
	while( i < csv.size() )
	{
		...
		int xGst[4] = { csv[i+3], csv[i+5], csv[i+7], csv[i+9] };
		int yGst[4] = { csv[i+4], csv[i+6], csv[i+8], csv[i+10] };
		int tempMap[20][20];
		int count = 0;
		for( int x = 0; x < 20; x++ )
			for( int y = 0; y < 20; y++ )
			{
				tempMap[x][y] = csv[i+18+count];
				count++;
			}
		S = PacState(csv[i+1], csv[i+2], csv[i+12], csv[i+14], csv[i+13], csv[i+11], csv[temp+18], xGst, yGst, csv[i+15], csv[i+16], tempMap );
...
/** /		
		sprintf_s( out, 99, "%d", (int)temptime );
		print("_\tTime: "); println(out);
/**/
		tree_GstMv->Destroy();
		fflush(fileout);
