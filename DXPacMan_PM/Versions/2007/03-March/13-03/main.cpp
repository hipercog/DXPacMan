...
void DrawDots();
void InitGameAgents();
int GhostDriver( int xPos, int yPos );
...
//	unsigned int sTime = GetTickCount();
	...
	if( in == 'A' || in == 'a' )
	{
		...
		unsigned int pacTik = 96, gstTik = pacTik*2, pacRndrTime = eTime, gstRndrTime = eTime;
		bool animate = false;
...
{
	//"Shadow" Ghost Sprite
	SetSpritePosition(&g_ShadowSpritePos);
	newRandom[0] = 0;	newRandom[1] = 1;	newRandom[2] = 2;	newRandom[3] = 3;

	if( NUM_GS > 1 )
	{
	//"Speedy" Ghost Sprite
	SetSpritePosition(&g_SpeedySpritePos);
	newRandom[0] = 0;	newRandom[1] = 1;	newRandom[2] = 2;	newRandom[3] = 3;

	if( NUM_GS > 2 )
	{
	//"Pokey" Ghost Sprite
	SetSpritePosition(&g_PokeySpritePos);
	newRandom[0] = 0;	newRandom[1] = 1;	newRandom[2] = 2;	newRandom[3] = 3;

	if( NUM_GS > 3 )
	//"Bashful" Ghost Sprite
	SetSpritePosition(&g_BashfulSpritePos);
	}}
}
...
//	if( manhattanDist( xPos, yPos, xIndex, yIndex ) < 12 ) GDMode = 0;
	...
	case 0:
	{
		//Aggressive Ghosts - chase when GhostAttack, run when PacAttack
		int dirs[4] = {INT_MAX, INT_MAX, INT_MAX, INT_MAX};
		
		if( PacMap[xPos][yPos-1] > 1 )
		{
			dirCnt++;
			dirs[0] = myHeuristicDist( xPos, yPos-1, xIndex, yIndex );
		}
		if( PacMap[xPos][yPos+1] > 1 )
		{
			dirCnt++;
			dirs[1] = myHeuristicDist( xPos, yPos+1, xIndex, yIndex );
		}

		//If this if evaluates both parts of the statement when a ghost is on a teleporter, we get an error
		if( PacMap[xPos][yPos] != 3 && PacMap[xPos+1][yPos] == 1 )
		{
			//nothing
		}else{
			dirCnt++;
			dirs[2] = myHeuristicDist( xPos+1, yPos, xIndex, yIndex );
		}
		if( PacMap[xPos][yPos] != 3 && PacMap[xPos-1][yPos] == 1 )
		{
			//nothing
			...
			dirCnt++;
			dirs[3] = myHeuristicDist( xPos-1, yPos, xIndex, yIndex );
		}
/*
		if( PacMap[xPos][yPos] == 3 )
		{
			dirCnt++;
			dirs[2] = myHeuristicDist( xPos+1, yPos, xIndex, yIndex );
		}
		else if( PacMap[xPos+1][yPos] > 1 )
		{
			dirCnt++;
			dirs[2] = myHeuristicDist( xPos+1, yPos, xIndex, yIndex );
		}
...
//		GDMode = 1;
	...
	case 1:
	{
		...
		case 2:
		{
			if( xPos < 19 && PacMap[xPos+1][yPos] == 1 )
			{
		...
		case 3:
		{
			if( xPos > 0 && PacMap[xPos-1][yPos] == 1 )
			{
...
void SetSpritePosition( LPD3DXVECTOR3 pSpritePos )//, int nChoice)
{
	...
	switch( GhostDriver( xPosIndex, yPosIndex ) )
	{
	case 0:
	{
		yPosFinal--;
