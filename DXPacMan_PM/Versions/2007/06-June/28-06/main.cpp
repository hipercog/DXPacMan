...
HRESULT GameInit(HWND hWnd)
{
	...
	if (FAILED(hr = LoadBitmapToSurface("resources/screen2.bmp", &g_pScreenSurface, g_pDevice)))
		return hr;	
...
HRESULT LoadMapFromFile()
{
	...
	ifstream in("resources/map1.txt");
	if (!in)
		return E_FAIL;
...
/**/
	...
	case 0:
	{
		...
		{
			dirCnt++;
			dists[0] = myHeuristicDist( xPos, yPos-1, xIndex, yIndex );
		}
		if( PacMap[xPos][yPos+1] > 1 )
		{
			dirCnt++;
			dists[1] = myHeuristicDist( xPos, yPos+1, xIndex, yIndex );
		}
		...
		if( PacMap[xPos][yPos] != 3 && PacMap[xPos+1][yPos] == 1 )
		{
			...
			dirCnt++;
			dists[2] = myHeuristicDist( xPos+1, yPos, xIndex, yIndex );
		}
		if( PacMap[xPos][yPos] != 3 && PacMap[xPos-1][yPos] == 1 )
		{
			//nothing
			...
			dirCnt++;
			dists[3] = myHeuristicDist( xPos-1, yPos, xIndex, yIndex );
		}
