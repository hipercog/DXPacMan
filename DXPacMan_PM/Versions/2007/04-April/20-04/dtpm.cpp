...
/**/
	...
	while( i < csv.size() )
	{
		...
		S = PacState(csv[i+1], csv[i+2], csv[i+addend+2], csv[i+addend+4], csv[i+addend+3], csv[i+addend+1], csv[temp+addend+8], xGst, yGst, csv[i+addend+5] == 1, csv[i+addend+6] == 1, tempMap );
		i = ++temp;
/**/	
		buildTreeOfGhostMoves( lookahead, -1, -1, S );
		iter_GstMv.Root();
		...
		sprintf_s( out, 99, "%d", pred ); //Print Predictions first
		print(out); print(",");

		sprintf_s( out, 99, "%d", S.MovedDir ); //Print Moves second
		print(out); print(",");

		if( pred == S.MovedDir ) //Print match or no match
			println( "1" );
		else
			println( "0" );
...
/**/
		...
		for( int count = 0; count < POT_GS; count++ )
		{
			...
			//if( preCalculatedUtilsDataStruct[Pac(x,y)][g.1(x,y)]..[g.NUM_GS(x,y)].utility == 0 )
			sum[count] = ( utility(S) * gstPrbs ) / 100;
			//else sum[count] = S.utility
			S = tempPS;
			gstPrbs = 1;
		}
	}

	return sum[ highest(sum, POT_GS) ];
}
