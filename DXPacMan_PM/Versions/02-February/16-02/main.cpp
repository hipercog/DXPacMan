...
//Path for when this code is being operated automatically
char* path;
...
/*	Test Harness allows isolated testing of new code. */
	if( TEST_HARNESS )
	{
		sleep( 2000 );
...
/*	If not using test harness, we are either recording a played game, in release mode, or analysing a log in debug mode. */
...
/*	Do in all cases - this string will time stamp every file we output	*/
	gameDir = out;

/*	Load a deque with rule strings	*/
	InitStrings();
...
/* Add a console to run alongside our main window, taking keyboard I/O */
	...
	{
		RedirectIOToConsole();	
		int in = 4, bt = 6, alpha = 90, ID = 0;
/** /
		cout << "Enter depth of tree search: ";
		cin >> in;
		cout << "Enter back tracking lag: ";
		cin >> bt;
		...
		cin >> ID;
/**/
		sprintf_s( out, 99, "%d", in );
		logData += out; logData += ",bt-";
		sprintf_s( out, 99, "%d", bt );
		logData += out; logData += ",A-";
		sprintf_s( out, 99, "%d", alpha );
		logData += out; logData += ",ID-";
		sprintf_s( out, 99, "%d", ID );
		logData += out;
/**/
//		getPlayerData( logData );		// Initialise a file to write output to.
		print( "Depth:," ); print( logData.c_str() ); print("\n");
		logData.clear();

		DWORD temptime = GetTickCount();
		postDTPM( in, bt, alpha, ID, path );	//atoi( &in ) );
		temptime = GetTickCount() - temptime;
		sprintf_s( out, 99, "%d", temptime ); print("\nTime Lapse:,"); print(out);
		sleep( 100 );
		return 0;
	}
...
/**/		
	...
	//Check if all the pills have been eaten (therefore level is over)
...
void ResetAttackSwitch()
{
	...
	GhostAttack = true;
	gstTik -= (float)pacTik * 1;
	PacMap[9][8 + (Level%2)] = PacMap[10][8 + (Level%2)] = 2;
	if( Level < 12 )
		GhostFleeTimer = 340 - (Level*(11 - NumPills));
	else
		GhostFleeTimer = 150 + (Level*(11 - NumPills));
	RunOfGhostKills = 1;
...
HRESULT ChangeLevel()
{
	...
	Level++;
	gstTik += (float)pacTik * 0.2;
	if(Level%5 == 0)
		gstTik -= (float)pacTik * 0.11 * (float)(Level / 5);//pacTik * (2.5 - (Level / 100.0));
	if(Level == 1 || Level == 3 || Level == 6 || Level == 10 || Level == 15)
	{
		gakProb++;
		pacProb--;
	}
...
//			print("\n****  MOVED WEST  ****\n");
	...
	else
	{
		...
		else if(PacMap[xPosIndex][yPosIndex] == 5 && GhostAttack)
		{
			PacMap[xPosIndex][yPosIndex] = 6; //code for eaten dot
...
void CollisionCheck()
{
	...
	if(g_pCherrySprite != NULL)
	{
		if( TRUE == CheckForCollision(&g_PacmanSpritePos, &g_CherrySpritePos) && !CherryEaten )
		{
			bCherryGone = true;
			GameLives++;
		}else 
		if (GhostAttack && CheckForCollision(&g_ShadowSpritePos, &g_CherrySpritePos))
			bCherryGone = true;
...
/**/
	...
	if( bPacManGone )
	{
		...
		//Decrease the minimum time at which Pacman gets the Cherry
		if( CherryEaten && randCherryInit != INT_MAX )
			randCherryInit -= 250;
	}
...
HRESULT DrawEnd()
{
	...
	//Initialise the RECT structure for the "Esc To Exit" text
	FontRect.left		= 170;
	FontRect.top		= 400;
	FontRect.right		= 350;
	FontRect.bottom		= 440;
