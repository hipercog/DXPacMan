 ...
 *	Global fields.
 */
int inc = 1, Lvl = 0, lastCycle = 0, matches = 0;
...
/**/
		...
		sprintf_s( out, 99, "%d", predir == real_State.MovedDir );
		print( ",Match:," ); print( out );
		matches += predir == real_State.MovedDir;
		sprintf_s( out, 99, "%f", (float)(matches / (real_State.State + 1) ) );
