...
double utility( deque <PacState> States )
{
	...
	for( int i = S; i > max( 0, S-lookahead ); i-- )
	{
		...
		{
/**/
			util = util_A4_HuntAfterPill( States );
			utility += util;
/** /
			sprintf_s( out, 99, "%f", util );
			print( "HuntAfter:," ); print( out );
/**/
			util = util_A6_Chase¬Dots( States );
			utility += util;
/** /
			sprintf_s( out, 99, "%f", util );
			print( "Chase¬Dots:," ); print( out );
/**/
			util = util_C4_CaughtAfterHunt( States );
			utility += util;
/** /
			sprintf_s( out, 99, "%f", util );
			print( ",CaughtAfterHunt:," ); print( out );
/**/
			break;
		}else{
/**/
			util = util_C1_ThreatPrcptn( States );
			utility += util;
/** /
			sprintf_s( out, 99, "%f", util );
			print( ",ThreatPrcptn:," ); print( out );
/**/
			util = util_C3_CloseCalls( States );
			utility += util;
/** /
			sprintf_s( out, 99, "%f", util );
			print( ",CloseCalls:," ); print( out );
/**/
			break;
		}
	}
...
/**/
	util = util_Agg1_LivesSDev( States );
	utility += util;
/** /
	sprintf_s( out, 99, "%f", util );
	print( ",LivesSDev:," ); print( out );
/**/
	util = util_Agg2_ChryBoolSum( States );
	utility += util;
/** /
	sprintf_s( out, 99, "%f", util );
	print( ",CountUntilCherryEaten:," ); print( out );
/**/
	util = util_S2_LivesChng( States );
	utility += util;
/** /
	sprintf_s( out, 99, "%f", util );
	print( ",LivesChange:," ); print( out ); print( "," );
/** /
	for( int i = 0; i < s; i++ )
	{
		pS = States[i];
