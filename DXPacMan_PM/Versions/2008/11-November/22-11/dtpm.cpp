...
int spawnStates( Tree<PacState>* tree );
...
/**/
	...
	while( i < csv.size() )
	{
		...
		i = ++temp;
/**/
		buildTreeOfStates( lookahead, S );
		iter_States.Root();
//		exploreStates( lookahead, S );
...
GhostMoves initNextMove_Ghost( PacState pS )	//int *baseGhostX, int *baseGhostY /*[Ghost 1-4][x-y]*/, int Map[20][20])
{
	...
	for( int g = 0; g < NUM_GS; g++ )
	{
		...
		{
			gDirs[0] = -1;
			gDist[0] = myHeuristicDist( pS.xGhost[g], pS.yGhost[g] - 1, S.xPac, S.yPac );
		}
		if( pS.dtpMap[pS.xGhost[g]][pS.yGhost[g] + 1 ] != 1 )
		{
			gDirs[1] = 1;
			gDist[1] = myHeuristicDist( pS.xGhost[g], pS.yGhost[g] + 1, S.xPac, S.yPac );
		}
		if( pS.dtpMap[pS.xGhost[g] + 1 ][pS.yGhost[g]] != 1 )
		{
			gDirs[2] = 1;
			gDist[2] = myHeuristicDist( pS.xGhost[g] + 1, pS.yGhost[g], S.xPac, S.yPac );
		}
		if( pS.dtpMap[pS.xGhost[g] - 1 ][pS.yGhost[g]] != 1 )
		{
			gDirs[3] = -1;
			gDist[3] = myHeuristicDist( pS.xGhost[g] - 1, pS.yGhost[g], S.xPac, S.yPac );
		}
...
{
	double prob;
	//Prob of the Pacman action and all the Ghosts' actions, for one iteration past the current Pacman state.
	return prob;
}
...
void buildTreeOfStates( unsigned int recurseLimit, PacState pS )
{
	...
	tree_States = new Tree<PacState>(pS);
	...
	PreorderToDepth( tree_States, spawnStates, recurseLimit );
}
...
int spawnStates( Tree<PacState>* tree )
{
	...
	if( gm.perms > 0 && nm.prob > 0 )
	{
		...
		for( int pot = 0; pot < POT_GS; pot++ )
			if( gm.ghostXYMove[0][2][pot] > 0 )	//SHOULD USE NOMV HERE
			{
	...
	//print("\nNEW CHILDREN BEING GENERATED\n");
		for( int count = 0; count < POT_GS; count++ )
		{
			if( gM.ghostXYMove[0][0][count] > NOMV )
			{
				...
				initNextMove_Ghost( ghostBranchX, ghostBranchY, S.dtpMap );
...
/**/
	return inc; //tree_States->m_children.m_count;
}
