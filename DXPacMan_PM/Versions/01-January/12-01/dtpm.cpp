...
void postDTPM( unsigned int look_AHead )
{
	...
	deque<int> csv = readFromCSVFile( "stateData/chrisFullGame-1.csv" );
/**/
	int tempMap[20][20], xGst[NUM_GS], yGst[NUM_GS];
	int addend = 2 + (NUM_GS * 2);
	int i = 0, predir = -1, prdir2 = -1, compdir = -1, cmpdir2 = -1, temp, count, magnitude = 20;
	float matches = 0.0, matchs2 = 0.0, accuracy = 1.0, acc2 = 1.0, old_acc = 0.5, tendency = 10.0;
