...
cout << "P3 ok\t";
	P4a_HuntGst1_AvgSpd.push_back( plan4[0] /
							max(1, min(S5_GhostHunts[lvl], A3_GhostKills[lvl])) );	// 21	2	62 - 349	|				| 0.178 - 1	|

	P4b_HuntGst2_AvgSpd.push_back( plan4[2] / max(1, A3_GhostKills[lvl]-4) );		// 22	2	0 - 2307	|				| 0 - 1		|
...
cout << "P7 ok\t";
	...
	Points_Max[lvl-1] = arg[s-1].Points - sum( Points_Max );						// 24	3	3200-62910	| 1303 - 4194	| 0.311 - 1	|
 ...
 */
	res_Nve.push_back( A1_AtkGstHse[lvl] );
	res_Nve.push_back( A3_GhostKills[lvl] );
	res_Nve.push_back( A4_HuntAfterPill[lvl] );
	res_Nve.push_back( A6_Chase¬Dots[lvl] );
	res_Nve.push_back( C1b_ThreatPerception_NumKilled[lvl] );
	res_Nve.push_back( C2b_AvgDstPacAtkCntrd[lvl] );
	res_Nve.push_back( C3_CloseCalls[lvl] );
	res_Nve.push_back( C4_CaughtAfterHunt[lvl] );
	res_Nve.push_back( C5_MvsWoutGain[lvl] );
	res_Nve.push_back( C6_PntsOvrLives[lvl] );
	res_Nve.push_back( C7_KilledAtHouse[lvl] );
	res_Nve.push_back( ChryBool_Sum[lvl] );
	res_Nve.push_back( CycleCountPerState_SDev[lvl] );
	res_Nve.push_back( D1_CyclesPerMove[lvl] );
	res_Nve.push_back( D2_Osclltng[lvl] );
	res_Nve.push_back( Lives_SDev[lvl] );
	res_Nve.push_back( P1_LureToPill[lvl] );
	res_Nve.push_back( P1b_2GstsLrd2Pll[lvl] );
	res_Nve.push_back( P1c_GstsEtnOnPll[lvl] );
	res_Nve.push_back( P1d_PacEtnB4GetPll[lvl] );
	res_Nve.push_back( P3_CntDtsB4Pll1[lvl] );
	res_Nve.push_back( P4a_HuntGst1_AvgSpd[lvl] );
	res_Nve.push_back( P4b_HuntGst2_AvgSpd[lvl] );
	res_Nve.push_back( P7_PutOffGstHse[lvl] );
	res_Nve.push_back( Points_Max[lvl-1] );
	res_Nve.push_back( R1_AvgTimeToEatChrry[lvl] );
	res_Nve.push_back( R2_AvgChrryPerLvl[lvl] );
	res_Nve.push_back( S1b_AvgStates[lvl] );
	res_Nve.push_back( S1c_AvgCycles[lvl] );
	res_Nve.push_back( S2a_LivesGained[lvl] );
	res_Nve.push_back( S2b_LivesLost[lvl] );
	res_Nve.push_back( S4_TeleportUse[lvl] );
	res_Nve.push_back( S5_GhostHunts[lvl] );
/** /
...
//////////////////////////////////////////////////////////////////////////////
//		THE NEW WAY OF DOING IT: PICKING JUST THE FIRST FIRING RULE			//
//////////////////////////////////////////////////////////////////////////////
...
/**/
	classification = OneRuleClassify_Averages( lvl );
	...
	fired.clear();
	fired.assign( 50, 0 );	// +1 is Conq rule firing, -1 is Not_Conq rule firing, 0 is no fire
	notcq = 0, conq = 0, classification = 0;
/**/
	classification = ClassifyConqueror_Averages( lvl );
	...
	if( lvl > 4 ) {
		...
		classification = OneRuleClassify_WindowAvg( lvl );
		...
		notcq = 0, conq = 0, classification = 0;
/**/
		classification = ClassifyConqueror_WindowAvg( lvl );
	...
	classification = OneRuleClassify_Naive( lvl );
	...
	classification = ClassifyConqueror_Naive( lvl );
	...
	logData += "Feature,Avg_Val,use_Avg,Wndw_Val,use_Wdw,Naive_Val,use_Nve\n";
...
/** /
	...
	file += "/";
/**/
	file += "\\avgNve_ByRank_Csfy-";
	sprintf_s(out, 99, "%d", lvl);
	file += out;
	file += ".csv";

	if( fopen_s( &stateFile, file.c_str(), "wt" ) != 0 )
		cout << "The file " << file.c_str() << " was not opened." << endl;
...
/*	First rule to fire wins		*/
	if( C5_MvsWoutGain[0] <= 150 && A4_HuntAfterPill[0] > 0.936225 && C2b_AvgDstPacAtkCntrd[0] <= 13.26 && C4_CaughtAfterHunt[0] <= 0.6925 && ChryBool_Sum[0] > 22 ){ conq += 	0.4536	; usage_Wdw[8]++; usage_Wdw[2]++; usage_Wdw[5]++; usage_Wdw[7]++; usage_Wdw[11]++; fired[0]++; }
	else
	if( C5_MvsWoutGain[0] > 106 && P4b_HuntGst2_AvgSpd[0] <= 1163 && P7_PutOffGstHse[0] <= 0.2678 ){ conq -= 	0.393491803	; usage_Wdw[8]++; usage_Wdw[22]++; usage_Wdw[23]++; fired[1]--; }
	else
	if( A4_HuntAfterPill[0] > 0.914325 && S1b_AvgStates[0] > 304 && C3_CloseCalls[0] > 0.1665 && S4_TeleportUse[0] > 0.858 ){ conq -= 	0.374970588	; usage_Wdw[2]++; usage_Wdw[27]++; usage_Wdw[6]++; usage_Wdw[31]++; fired[2]--; }
	else
	if( C5_MvsWoutGain[0] > 106 && P7_PutOffGstHse[0] <= 0.2678 && C1b_ThreatPerception_NumKilled[0] <= 2.5 && A4_HuntAfterPill[0] <= 2.9346 ){ conq -= 	0.34442623	; usage_Wdw[8]++; usage_Wdw[23]++; usage_Wdw[4]++; usage_Wdw[2]++; fired[3]--; }
	else
	if( S2a_LivesGained[0] > 0.775 && S1c_AvgCycles[0] <= 5653 && Lives_SDev[0] <= 1.674 ){ conq += 	0.327836066	; usage_Wdw[29]++; usage_Wdw[28]++; usage_Wdw[15]++; fired[4]++; }
	else
	if( A4_HuntAfterPill[0] <= 3.08 && S2a_LivesGained[0] <= 0.78375 && Lives_SDev[0] <= 1.468 ){ conq -= 	0.327737705	; usage_Wdw[2]++; usage_Wdw[29]++; usage_Wdw[15]++; fired[5]--; }
	else
	if( S2a_LivesGained[0] > 0.775 && C4_CaughtAfterHunt[0] <= 0.69625 && S1c_AvgCycles[0] <= 6527 ){ conq += 	0.323647059	; usage_Wdw[29]++; usage_Wdw[7]++; usage_Wdw[28]++; fired[6]++; }
	else
	if( C5_MvsWoutGain[0] > 78 && C3_CloseCalls[0] > 0.1545 ){ conq -= 	0.311442623	; usage_Wdw[8]++; usage_Wdw[6]++; usage_Wdw[6]++; fired[7]--; }
	else
	if( Lives_SDev[0] <= 1.54 && A4_HuntAfterPill[0] > 1.14975 && CycleCountPerState_SDev[0] <= 15.7575 && C4_CaughtAfterHunt[0] <= 0.45 ){ conq += 	0.294264706	; usage_Wdw[15]++; usage_Wdw[2]++; usage_Wdw[12]++; usage_Wdw[7]++; fired[8]++; }
	else
	if( S1c_AvgCycles[0] <= 4396 && A4_HuntAfterPill[0] > 0.99 && S2a_LivesGained[0] > 0.53625 ){ conq += 	0.294117647	; usage_Wdw[28]++; usage_Wdw[2]++; usage_Wdw[29]++; fired[9]++; }
	else
	if( GameOver && A4_HuntAfterPill[0] > 0.947175 && S5_GhostHunts[l] <= 3.764 && A6_Chase¬Dots[0] <= 5.3 && ChryBool_Sum[0] > 24 ){ conq += 	0.2933	; usage_Wdw[2]++; usage_Wdw[32]++; usage_Wdw[3]++; usage_Wdw[11]++; fired[10]++; }
	else
	if( C6_PntsOvrLives[0] <= 1897 && Lives_SDev[0] <= 1.54 && S2a_LivesGained[0] > 0.775 ){ conq += 	0.279529412	; usage_Wdw[9]++; usage_Wdw[15]++; usage_Wdw[29]++; fired[11]++; }
	else
	if( C5_MvsWoutGain[0] > 78 && ChryBool_Sum[0] > 23 && R1_AvgTimeToEatChrry[0] > 36 && A4_HuntAfterPill[0] <= 2.4966 ){ conq -= 	0.278770492	; usage_Wdw[8]++; usage_Wdw[11]++; usage_Wdw[25]++; usage_Wdw[2]++; fired[12]--; }
	else
	if( ChryBool_Sum[0] > 23 && S2a_LivesGained[0] > 0.775 && Lives_SDev[0] <= 1.53 ){ conq += 	0.2667	; usage_Wdw[11]++; usage_Wdw[29]++; usage_Wdw[15]++; fired[13]++; }
	else
	if( Lives_SDev[0] > 1.5 ){ conq -= 	0.235264706	; usage_Wdw[15]++; fired[14]--; }
	else
	if( A4_HuntAfterPill[0] > 2.5185 && C4_CaughtAfterHunt[0] <= 0.69625 ){ conq += 	0.226733333	; usage_Wdw[2]++; usage_Wdw[7]++; fired[15]++; }
	else
	if( R2_AvgChrryPerLvl[0] > 0.333 && P7_PutOffGstHse[0] <= 0.166732 && C1b_ThreatPerception_NumKilled[0] <= 1 && S2b_LivesLost[0] <= 0.737 ){ conq -= 	0.226133647	; usage_Wdw[26]++; usage_Wdw[23]++; usage_Wdw[4]++; usage_Wdw[30]++; fired[16]--; }
	else
	if( Points_Max[l-1] > 1950 && P3_CntDtsB4Pll1[0] > 4 && P4a_HuntGst1_AvgSpd[0] <= 236 ){ conq -= 	0.220705882	; usage_Wdw[24]++; usage_Wdw[20]++; usage_Wdw[21]++; fired[17]--; }
	else
	if( R2_AvgChrryPerLvl[0] > 0.333 && P7_PutOffGstHse[0] <= 0.250098 && P7_PutOffGstHse[0] >= 0.166732 ){ conq -= 	0.219350147	; usage_Wdw[26]++; usage_Wdw[23]++; usage_Wdw[23]++; fired[18]--; }
	else
	if( D2_Osclltng[0] > 122 ){ conq -= 	0.2133	; usage_Wdw[14]++; fired[19]--; }
	else
	if( A4_HuntAfterPill[0] <= 1.16 ){ conq -= 	0.213213115	; usage_Wdw[2]++; fired[20]--; }
	else
	if( C5_MvsWoutGain[0] <= 106 && S2a_LivesGained[0] > 0.775 ){ conq += 	0.213196721	; usage_Wdw[8]++; usage_Wdw[29]++; fired[21]++; }
	else
	if( A4_HuntAfterPill[0] <= 3.08 && R2_AvgChrryPerLvl[0] <= 0.802 && A3_GhostKills[0] > 0.46 ){ conq -= 	0.213114754	; usage_Wdw[2]++; usage_Wdw[26]++; usage_Wdw[1]++; fired[22]--; }
	else
	if( GameOver && A4_HuntAfterPill[0] > 0.947175 && S5_GhostHunts[l] > 3.764 && CycleCountPerState_SDev[0] > 13 ){ conq -= 	0.2	; usage_Wdw[2]++; usage_Wdw[32]++; usage_Wdw[12]++; fired[23]--; }
	else
	if( A4_HuntAfterPill[0] <= 3.08 && R2_AvgChrryPerLvl[0] <= 0.802 && A3_GhostKills[0] <= 0.46 && P1c_GstsEtnOnPll[0] <= 0.6588 ){ conq -= 	0.196688525	; usage_Wdw[2]++; usage_Wdw[26]++; usage_Wdw[1]++; usage_Wdw[18]++; fired[24]--; }
	else
	if( A4_HuntAfterPill[0] > 0.914325 && A1_AtkGstHse[0] <= 0.4288 && A6_Chase¬Dots[0] <= 4.843 && P1c_GstsEtnOnPll[0] > 0.5 ){ conq += 	0.19175	; usage_Wdw[2]++; usage_Wdw[0]++; usage_Wdw[3]++; usage_Wdw[18]++; fired[25]++; }
	else
	if( A4_HuntAfterPill[0] >= 0.914 && S1b_AvgStates[0] <= 304 ){ conq += 	0.191264706	; usage_Wdw[2]++; usage_Wdw[27]++; fired[26]++; }
	else
	if( A4_HuntAfterPill[0] > 0.7446 && S1b_AvgStates[0] <= 305 && C6_PntsOvrLives[0] <= 1912 ){ conq += 	0.176455882	; usage_Wdw[2]++; usage_Wdw[27]++; usage_Wdw[9]++; fired[27]++; }
	else
	if( GameOver && S1b_AvgStates[0] <= 394 && C4_CaughtAfterHunt[0] > 0.445 && S5_GhostHunts[l] > 3.644 ){ conq -= 	0.174110588	; usage_Wdw[27]++; usage_Wdw[7]++; usage_Wdw[32]++; fired[28]--; }
	else
	if( A4_HuntAfterPill[0] <= 3.08 && S2a_LivesGained[0] > 0.78375 && Lives_SDev[0] <= 1.55 && P1_LureToPill[0] > 0.08277 ){ conq += 	0.163934426	; usage_Wdw[2]++; usage_Wdw[29]++; usage_Wdw[15]++; usage_Wdw[16]++; fired[29]++; }
	else
	if( R2_AvgChrryPerLvl[0] > 0.333 && P7_PutOffGstHse[0] > 0.250098 && Lives_SDev[0] <= 1.52 ){ conq += 	0.163206441	; usage_Wdw[26]++; usage_Wdw[23]++; usage_Wdw[15]++; fired[30]++; }
	else
	if( Points_Max[l-1] <= 1870 ){ conq += 	0.161826824	; usage_Wdw[24]++; fired[31]++; }
	else
	if( A4_HuntAfterPill[0] > 3.08 && S1c_AvgCycles[0] <= 5653 ){ conq += 	0.161764706	; usage_Wdw[2]++; usage_Wdw[28]++; fired[32]++; }
	else
	if( A4_HuntAfterPill[0] <= 3.08 && S2a_LivesGained[0] > 0.775 && D1_CyclesPerMove[0] <= 15.6 && Lives_SDev[0] <= 1.4 ){ conq += 	0.161764706	; usage_Wdw[2]++; usage_Wdw[29]++; usage_Wdw[13]++; usage_Wdw[15]++; fired[33]++; }
	else
	if( P1d_PacEtnB4GetPll[0] <= 0 && C1b_ThreatPerception_NumKilled[0] <= 2 && R2_AvgChrryPerLvl[0] > 0.333 && P1c_GstsEtnOnPll[0] <= 2.1528 && P1b_2GstsLrd2Pll[0] <= 0.167 && P4a_HuntGst1_AvgSpd[0] <= 223 && R2_AvgChrryPerLvl[0] <= 0.750 && A6_Chase¬Dots[0] > 1.4 ){ conq -= 	0.161764706	; usage_Wdw[19]++; usage_Wdw[4]++; usage_Wdw[26]++; usage_Wdw[18]++; usage_Wdw[17]++; usage_Wdw[21]++; usage_Wdw[26]++; usage_Wdw[3]++; fired[34]--; }
	else
	if( A4_HuntAfterPill[0] > 3.08 ){ conq += 	0.147540984	; usage_Wdw[2]++; fired[35]++; }
	else
	if( S1c_AvgCycles[0] > 5500 ){ conq -= 	0.147540984	; usage_Wdw[28]++; fired[36]--; }
	else
	if( A4_HuntAfterPill[0] > 1.16 && ChryBool_Sum[0] <= 23 ){ conq -= 	0.147540984	; usage_Wdw[2]++; usage_Wdw[11]++; fired[37]--; }
	else
	if( ChryBool_Sum[0] <= 23 ){ conq -= 	0.1333	; usage_Wdw[11]++; fired[38]--; }
	else
	if( Points_Max[l-1] <= 1878 && P4b_HuntGst2_AvgSpd[0] <= 39 ){ conq += 	0.132352941	; usage_Wdw[24]++; usage_Wdw[22]++; fired[39]++; }
	else
	if( R2_AvgChrryPerLvl[0] > 0.333 && P7_PutOffGstHse[0] <= 0.166732 && C1b_ThreatPerception_NumKilled[0] > 1 && S4_TeleportUse[0] <= 1.923 ){ conq += 	0.131411765	; usage_Wdw[26]++; usage_Wdw[23]++; usage_Wdw[4]++; usage_Wdw[31]++; fired[40]++; }
	else
	if( A4_HuntAfterPill[0] <= 3.08 && S2a_LivesGained[0] > 0.78375 && Lives_SDev[0] > 1.55 ){ conq -= 	0.131163934	; usage_Wdw[2]++; usage_Wdw[29]++; usage_Wdw[15]++; fired[41]--; }
	else
	if( D2_Osclltng[0] <= 123 && R1_AvgTimeToEatChrry[0] <= 21 ){ conq -= 	0.126575738	; usage_Wdw[14]++; usage_Wdw[25]++; fired[42]--; }
	else
	if( ChryBool_Sum[0] > 23 && C3_CloseCalls[0] <= 0.174 ){ conq += 	0.12	; usage_Wdw[11]++; usage_Wdw[25]++; fired[43]++; }
	else
	if( ChryBool_Sum[0] > 23 && C3_CloseCalls[0] > 0.174 && A4_HuntAfterPill[0] > 3.104325 ){ conq += 	0.119973333	; usage_Wdw[11]++; usage_Wdw[6]++; usage_Wdw[2]++; fired[44]++; }
	else
	if( S2a_LivesGained[0] <= 0.775 && A4_HuntAfterPill[0] > 1.407075 && A6_Chase¬Dots[0] <= 1.447 ){ conq += 	0.117647059	; usage_Wdw[29]++; usage_Wdw[2]++; usage_Wdw[3]++; fired[45]++; }
	else
	if( A4_HuntAfterPill[0] > 0.914325 && A1_AtkGstHse[0] > 0.4288 && P1d_PacEtnB4GetPll[0] <= 0 && Lives_SDev[0] <= 1.52 && C3_CloseCalls[0] <= 0.5385 && C7_KilledAtHouse[0] <= 0.222 ){ conq += 	0.113455882	; usage_Wdw[2]++; usage_Wdw[0]++; usage_Wdw[19]++; usage_Wdw[15]++; usage_Wdw[6]++; usage_Wdw[10]++; fired[46]++; }
	else
	if( C5_MvsWoutGain[0] > 150 ){ conq -= 	0.1067	; usage_Wdw[8]++; fired[47]--; }
	else
	if( P7_PutOffGstHse[0] > 0.375 ){ conq += 	0.106509836	; usage_Wdw[23]++; fired[48]++; }
	else
	if( ChryBool_Sum[0] > 22 && A6_Chase¬Dots[0] > 5.25 ){ conq -= 	0.102941176	; usage_Wdw[11]++; usage_Wdw[3]++; fired[49]--; }
...
/*	First rule to fire wins		*/
	if( C5_MvsWoutGain[0] <= 150 && A4_HuntAfterPill[0] > 0.936225 && C2b_AvgDstPacAtkCntrd[0] <= 13.26 && C4_CaughtAfterHunt[0] <= 0.6925 && ChryBool_Sum[0] > 22 ){ conq += 	0.4536	; usage_Avg[8]++; usage_Avg[2]++; usage_Avg[5]++; usage_Avg[7]++; usage_Avg[11]++; fired[0]++; count++; if( count == 3 ) return conq; }
	
	if( C5_MvsWoutGain[0] > 106 && P4b_HuntGst2_AvgSpd[0] <= 1163 && P7_PutOffGstHse[0] <= 0.2678 ){ conq -= 	0.393491803	; usage_Avg[8]++; usage_Avg[22]++; usage_Avg[23]++; fired[1]--; count++; if( count == 3 ) return conq; }
	
	if( A4_HuntAfterPill[0] > 0.914325 && S1b_AvgStates[0] > 304 && C3_CloseCalls[0] > 0.1665 && S4_TeleportUse[0] > 0.858 ){ conq -= 	0.374970588	; usage_Avg[2]++; usage_Avg[27]++; usage_Avg[6]++; usage_Avg[31]++; fired[2]--; count++; if( count == 3 ) return conq; }
	
	if( C5_MvsWoutGain[0] > 106 && P7_PutOffGstHse[0] <= 0.2678 && C1b_ThreatPerception_NumKilled[0] <= 2.5 && A4_HuntAfterPill[0] <= 2.9346 ){ conq -= 	0.34442623	; usage_Avg[8]++; usage_Avg[23]++; usage_Avg[4]++; usage_Avg[2]++; fired[3]--; count++; if( count == 3 ) return conq; }
	
	if( S2a_LivesGained[0] > 0.775 && S1c_AvgCycles[0] <= 5653 && Lives_SDev[0] <= 1.674 ){ conq += 	0.327836066	; usage_Avg[29]++; usage_Avg[28]++; usage_Avg[15]++; fired[4]++; count++; if( count == 3 ) return conq; }
	
	if( A4_HuntAfterPill[0] <= 3.08 && S2a_LivesGained[0] <= 0.78375 && Lives_SDev[0] <= 1.468 ){ conq -= 	0.327737705	; usage_Avg[2]++; usage_Avg[29]++; usage_Avg[15]++; fired[5]--; count++; if( count == 3 ) return conq; }
	
	if( S2a_LivesGained[0] > 0.775 && C4_CaughtAfterHunt[0] <= 0.69625 && S1c_AvgCycles[0] <= 6527 ){ conq += 	0.323647059	; usage_Avg[29]++; usage_Avg[7]++; usage_Avg[28]++; fired[6]++; count++; if( count == 3 ) return conq; }
	
	if( C5_MvsWoutGain[0] > 78 && C3_CloseCalls[0] > 0.1545 ){ conq -= 	0.311442623	; usage_Avg[8]++; usage_Avg[6]++; usage_Avg[6]++; fired[7]--; count++; if( count == 3 ) return conq; }
	
	if( Lives_SDev[0] <= 1.54 && A4_HuntAfterPill[0] > 1.14975 && CycleCountPerState_SDev[0] <= 15.7575 && C4_CaughtAfterHunt[0] <= 0.45 ){ conq += 	0.294264706	; usage_Avg[15]++; usage_Avg[2]++; usage_Avg[12]++; usage_Avg[7]++; fired[8]++; count++; if( count == 3 ) return conq; }
	
	if( S1c_AvgCycles[0] <= 4396 && A4_HuntAfterPill[0] > 0.99 && S2a_LivesGained[0] > 0.53625 ){ conq += 	0.294117647	; usage_Avg[28]++; usage_Avg[2]++; usage_Avg[29]++; fired[9]++; count++; if( count == 3 ) return conq; }
	
	if( GameOver && A4_HuntAfterPill[0] > 0.947175 && S5_GhostHunts[l] <= 3.764 && A6_Chase¬Dots[0] <= 5.3 && ChryBool_Sum[0] > 24 ){ conq += 	0.2933	; usage_Avg[2]++; usage_Avg[32]++; usage_Avg[3]++; usage_Avg[11]++; fired[10]++; count++; if( count == 3 ) return conq; }
	
	if( C6_PntsOvrLives[0] <= 1897 && Lives_SDev[0] <= 1.54 && S2a_LivesGained[0] > 0.775 ){ conq += 	0.279529412	; usage_Avg[9]++; usage_Avg[15]++; usage_Avg[29]++; fired[11]++; count++; if( count == 3 ) return conq; }
	
	if( C5_MvsWoutGain[0] > 78 && ChryBool_Sum[0] > 23 && R1_AvgTimeToEatChrry[0] > 36 && A4_HuntAfterPill[0] <= 2.4966 ){ conq -= 	0.278770492	; usage_Avg[8]++; usage_Avg[11]++; usage_Avg[25]++; usage_Avg[2]++; fired[12]--; count++; if( count == 3 ) return conq; }
	
	if( ChryBool_Sum[0] > 23 && S2a_LivesGained[0] > 0.775 && Lives_SDev[0] <= 1.53 ){ conq += 	0.2667	; usage_Avg[11]++; usage_Avg[29]++; usage_Avg[15]++; fired[13]++; count++; if( count == 3 ) return conq; }
	
	if( Lives_SDev[0] > 1.5 ){ conq -= 	0.235264706	; usage_Avg[15]++; fired[14]--; count++; if( count == 3 ) return conq; }
	
	if( A4_HuntAfterPill[0] > 2.5185 && C4_CaughtAfterHunt[0] <= 0.69625 ){ conq += 	0.226733333	; usage_Avg[2]++; usage_Avg[7]++; fired[15]++; count++; if( count == 3 ) return conq; }
	
	if( R2_AvgChrryPerLvl[0] > 0.333 && P7_PutOffGstHse[0] <= 0.166732 && C1b_ThreatPerception_NumKilled[0] <= 1 && S2b_LivesLost[0] <= 0.737 ){ conq -= 	0.226133647	; usage_Avg[26]++; usage_Avg[23]++; usage_Avg[4]++; usage_Avg[30]++; fired[16]--; count++; if( count == 3 ) return conq; }
	
	if( Points_Max[l-1] > 1950 && P3_CntDtsB4Pll1[0] > 4 && P4a_HuntGst1_AvgSpd[0] <= 236 ){ conq -= 	0.220705882	; usage_Avg[24]++; usage_Avg[20]++; usage_Avg[21]++; fired[17]--; count++; if( count == 3 ) return conq; }
	
	if( R2_AvgChrryPerLvl[0] > 0.333 && P7_PutOffGstHse[0] <= 0.250098 && P7_PutOffGstHse[0] >= 0.166732 ){ conq -= 	0.219350147	; usage_Avg[26]++; usage_Avg[23]++; usage_Avg[23]++; fired[18]--; count++; if( count == 3 ) return conq; }
	
	if( D2_Osclltng[0] > 122 ){ conq -= 	0.2133	; usage_Avg[14]++; fired[19]--; count++; if( count == 3 ) return conq; }
	
	if( A4_HuntAfterPill[0] <= 1.16 ){ conq -= 	0.213213115	; usage_Avg[2]++; fired[20]--; count++; if( count == 3 ) return conq; }
	
	if( C5_MvsWoutGain[0] <= 106 && S2a_LivesGained[0] > 0.775 ){ conq += 	0.213196721	; usage_Avg[8]++; usage_Avg[29]++; fired[21]++; count++; if( count == 3 ) return conq; }
	
	if( A4_HuntAfterPill[0] <= 3.08 && R2_AvgChrryPerLvl[0] <= 0.802 && A3_GhostKills[0] > 0.46 ){ conq -= 	0.213114754	; usage_Avg[2]++; usage_Avg[26]++; usage_Avg[1]++; fired[22]--; count++; if( count == 3 ) return conq; }
	
	if( GameOver && A4_HuntAfterPill[0] > 0.947175 && S5_GhostHunts[l] > 3.764 && CycleCountPerState_SDev[0] > 13 ){ conq -= 	0.2	; usage_Avg[2]++; usage_Avg[32]++; usage_Avg[12]++; fired[23]--; count++; if( count == 3 ) return conq; }
	
	if( A4_HuntAfterPill[0] <= 3.08 && R2_AvgChrryPerLvl[0] <= 0.802 && A3_GhostKills[0] <= 0.46 && P1c_GstsEtnOnPll[0] <= 0.6588 ){ conq -= 	0.196688525	; usage_Avg[2]++; usage_Avg[26]++; usage_Avg[1]++; usage_Avg[18]++; fired[24]--; count++; if( count == 3 ) return conq; }
	
	if( A4_HuntAfterPill[0] > 0.914325 && A1_AtkGstHse[0] <= 0.4288 && A6_Chase¬Dots[0] <= 4.843 && P1c_GstsEtnOnPll[0] > 0.5 ){ conq += 	0.19175	; usage_Avg[2]++; usage_Avg[0]++; usage_Avg[3]++; usage_Avg[18]++; fired[25]++; count++; if( count == 3 ) return conq; }
	
	if( A4_HuntAfterPill[0] >= 0.914 && S1b_AvgStates[0] <= 304 ){ conq += 	0.191264706	; usage_Avg[2]++; usage_Avg[27]++; fired[26]++; count++; if( count == 3 ) return conq; }
	
	if( A4_HuntAfterPill[0] > 0.7446 && S1b_AvgStates[0] <= 305 && C6_PntsOvrLives[0] <= 1912 ){ conq += 	0.176455882	; usage_Avg[2]++; usage_Avg[27]++; usage_Avg[9]++; fired[27]++; count++; if( count == 3 ) return conq; }
	
	if( GameOver && S1b_AvgStates[0] <= 394 && C4_CaughtAfterHunt[0] > 0.445 && S5_GhostHunts[l] > 3.644 ){ conq -= 	0.174110588	; usage_Avg[27]++; usage_Avg[7]++; usage_Avg[32]++; fired[28]--; count++; if( count == 3 ) return conq; }
	
	if( A4_HuntAfterPill[0] <= 3.08 && S2a_LivesGained[0] > 0.78375 && Lives_SDev[0] <= 1.55 && P1_LureToPill[0] > 0.08277 ){ conq += 	0.163934426	; usage_Avg[2]++; usage_Avg[29]++; usage_Avg[15]++; usage_Avg[16]++; fired[29]++; count++; if( count == 3 ) return conq; }
	
	if( R2_AvgChrryPerLvl[0] > 0.333 && P7_PutOffGstHse[0] > 0.250098 && Lives_SDev[0] <= 1.52 ){ conq += 	0.163206441	; usage_Avg[26]++; usage_Avg[23]++; usage_Avg[15]++; fired[30]++; count++; if( count == 3 ) return conq; }
	
	if( Points_Max[l-1] <= 1870 ){ conq += 	0.161826824	; usage_Avg[24]++; fired[31]++; count++; if( count == 3 ) return conq; }
	
	if( A4_HuntAfterPill[0] > 3.08 && S1c_AvgCycles[0] <= 5653 ){ conq += 	0.161764706	; usage_Avg[2]++; usage_Avg[28]++; fired[32]++; count++; if( count == 3 ) return conq; }
	
	if( A4_HuntAfterPill[0] <= 3.08 && S2a_LivesGained[0] > 0.775 && D1_CyclesPerMove[0] <= 15.6 && Lives_SDev[0] <= 1.4 ){ conq += 	0.161764706	; usage_Avg[2]++; usage_Avg[29]++; usage_Avg[13]++; usage_Avg[15]++; fired[33]++; count++; if( count == 3 ) return conq; }
	
	if( P1d_PacEtnB4GetPll[0] <= 0 && C1b_ThreatPerception_NumKilled[0] <= 2 && R2_AvgChrryPerLvl[0] > 0.333 && P1c_GstsEtnOnPll[0] <= 2.1528 && P1b_2GstsLrd2Pll[0] <= 0.167 && P4a_HuntGst1_AvgSpd[0] <= 223 && R2_AvgChrryPerLvl[0] <= 0.750 && A6_Chase¬Dots[0] > 1.4 ){ conq -= 	0.161764706	; usage_Avg[19]++; usage_Avg[4]++; usage_Avg[26]++; usage_Avg[18]++; usage_Avg[17]++; usage_Avg[21]++; usage_Avg[26]++; usage_Avg[3]++; fired[34]--; count++; if( count == 3 ) return conq; }
	
	if( A4_HuntAfterPill[0] > 3.08 ){ conq += 	0.147540984	; usage_Avg[2]++; fired[35]++; count++; if( count == 3 ) return conq; }
	
	if( S1c_AvgCycles[0] > 5500 ){ conq -= 	0.147540984	; usage_Avg[28]++; fired[36]--; count++; if( count == 3 ) return conq; }
	
	if( A4_HuntAfterPill[0] > 1.16 && ChryBool_Sum[0] <= 23 ){ conq -= 	0.147540984	; usage_Avg[2]++; usage_Avg[11]++; fired[37]--; count++; if( count == 3 ) return conq; }
	
	if( ChryBool_Sum[0] <= 23 ){ conq -= 	0.1333	; usage_Avg[11]++; fired[38]--; count++; if( count == 3 ) return conq; }
	
	if( Points_Max[l-1] <= 1878 && P4b_HuntGst2_AvgSpd[0] <= 39 ){ conq += 	0.132352941	; usage_Avg[24]++; usage_Avg[22]++; fired[39]++; count++; if( count == 3 ) return conq; }
	
	if( R2_AvgChrryPerLvl[0] > 0.333 && P7_PutOffGstHse[0] <= 0.166732 && C1b_ThreatPerception_NumKilled[0] > 1 && S4_TeleportUse[0] <= 1.923 ){ conq += 	0.131411765	; usage_Avg[26]++; usage_Avg[23]++; usage_Avg[4]++; usage_Avg[31]++; fired[40]++; count++; if( count == 3 ) return conq; }
	
	if( A4_HuntAfterPill[0] <= 3.08 && S2a_LivesGained[0] > 0.78375 && Lives_SDev[0] > 1.55 ){ conq -= 	0.131163934	; usage_Avg[2]++; usage_Avg[29]++; usage_Avg[15]++; fired[41]--; count++; if( count == 3 ) return conq; }
	
	if( D2_Osclltng[0] <= 123 && R1_AvgTimeToEatChrry[0] <= 21 ){ conq -= 	0.126575738	; usage_Avg[14]++; usage_Avg[25]++; fired[42]--; count++; if( count == 3 ) return conq; }
	
	if( ChryBool_Sum[0] > 23 && C3_CloseCalls[0] <= 0.174 ){ conq += 	0.12	; usage_Avg[11]++; usage_Avg[25]++; fired[43]++; count++; if( count == 3 ) return conq; }
	
	if( ChryBool_Sum[0] > 23 && C3_CloseCalls[0] > 0.174 && A4_HuntAfterPill[0] > 3.104325 ){ conq += 	0.119973333	; usage_Avg[11]++; usage_Avg[6]++; usage_Avg[2]++; fired[44]++; count++; if( count == 3 ) return conq; }
	
	if( S2a_LivesGained[0] <= 0.775 && A4_HuntAfterPill[0] > 1.407075 && A6_Chase¬Dots[0] <= 1.447 ){ conq += 	0.117647059	; usage_Avg[29]++; usage_Avg[2]++; usage_Avg[3]++; fired[45]++; count++; if( count == 3 ) return conq; }
	
	if( A4_HuntAfterPill[0] > 0.914325 && A1_AtkGstHse[0] > 0.4288 && P1d_PacEtnB4GetPll[0] <= 0 && Lives_SDev[0] <= 1.52 && C3_CloseCalls[0] <= 0.5385 && C7_KilledAtHouse[0] <= 0.222 ){ conq += 	0.113455882	; usage_Avg[2]++; usage_Avg[0]++; usage_Avg[19]++; usage_Avg[15]++; usage_Avg[6]++; usage_Avg[10]++; fired[46]++; count++; if( count == 3 ) return conq; }
	
	if( C5_MvsWoutGain[0] > 150 ){ conq -= 	0.1067	; usage_Avg[8]++; fired[47]--; count++; if( count == 3 ) return conq; }
	
	if( P7_PutOffGstHse[0] > 0.375 ){ conq += 	0.106509836	; usage_Avg[23]++; fired[48]++; count++; if( count == 3 ) return conq; }
	
	if( ChryBool_Sum[0] > 22 && A6_Chase¬Dots[0] > 5.25 ){ conq -= 	0.102941176	; usage_Avg[11]++; usage_Avg[3]++; fired[49]--; count++; if( count == 3 ) return conq; }
/**/
	return conq;
}
...
/*	First rule to fire wins		*/
	if( C5_MvsWoutGain[l] <= 150 && A4_HuntAfterPill[l] > 0 && C2b_AvgDstPacAtkCntrd[l] <= 13.26 && C4_CaughtAfterHunt[l] <= 2 && ChryBool_Sum[l] > 23 ){ conq += 	0.4536	; usage_Nve[8]++; usage_Nve[2]++; usage_Nve[5]++; usage_Nve[7]++; usage_Nve[11]++; fired[0]++; }
	else
	if( C5_MvsWoutGain[l] > 106 && P4b_HuntGst2_AvgSpd[l] <= 1164 && P7_PutOffGstHse[0] <= 0.469 ){ conq -= 	0.393491803	; usage_Nve[8]++; usage_Nve[22]++; usage_Nve[23]++; fired[1]--; }
	else
	if( A4_HuntAfterPill[l] > 0 && S1b_AvgStates[l] > 304 && C3_CloseCalls[l] > 0 && S4_TeleportUse[l] > 1 ){ conq -= 	0.374970588	; usage_Nve[2]++; usage_Nve[27]++; usage_Nve[6]++; usage_Nve[31]++; fired[2]--; }
	else
	if( C5_MvsWoutGain[l] > 106 && P7_PutOffGstHse[0] <= 0.469 && C1b_ThreatPerception_NumKilled[l] <= 2 && A4_HuntAfterPill[l] <= 2 ){ conq -= 	0.34442623	; usage_Nve[8]++; usage_Nve[23]++; usage_Nve[4]++; usage_Nve[2]++; fired[3]--; }
	else
	if( S2a_LivesGained[0] > 0.620 && S1c_AvgCycles[l] <= 5650 && Lives_SDev[l] <= 1.654 ){ conq += 	0.327836066	; usage_Nve[29]++; usage_Nve[28]++; usage_Nve[15]++; fired[4]++; }
	else
	if( A4_HuntAfterPill[l] <= 2 && S2a_LivesGained[0] <= 0.627 && Lives_SDev[l] <= 1.469 ){ conq -= 	0.327737705	; usage_Nve[2]++; usage_Nve[29]++; usage_Nve[15]++; fired[5]--; }
	else
	if( S2a_LivesGained[0] > 0.620 && C4_CaughtAfterHunt[l] <= 2 && S1c_AvgCycles[l] <= 6530 ){ conq += 	0.323647059	; usage_Nve[29]++; usage_Nve[7]++; usage_Nve[28]++; fired[6]++; }
	else
	if( C5_MvsWoutGain[l] > 79 && C3_CloseCalls[l] == 1 ){ conq -= 	0.311442623	; usage_Nve[8]++; usage_Nve[6]++; usage_Nve[6]++; fired[7]--; }
	else
	if( Lives_SDev[l] <= 1.54 && A4_HuntAfterPill[l] > 1 && CycleCountPerState_SDev[l] <= 15.77 && C4_CaughtAfterHunt[l] <= 1 ){ conq += 	0.294264706	; usage_Nve[15]++; usage_Nve[2]++; usage_Nve[12]++; usage_Nve[7]++; fired[8]++; }
	else
	if( S1c_AvgCycles[l] <= 4410 && A4_HuntAfterPill[l] > 1 && S2a_LivesGained[0] > 0.429 ){ conq += 	0.294117647	; usage_Nve[28]++; usage_Nve[2]++; usage_Nve[29]++; fired[9]++; }
	else
	if( GameOver && A4_HuntAfterPill[l] > 1 && S5_GhostHunts[l] <= 3.75 && A6_Chase¬Dots[l] <= 5 && ChryBool_Sum[l] > 25 ){ conq += 	0.2933	; usage_Nve[2]++; usage_Nve[32]++; usage_Nve[3]++; usage_Nve[11]++; fired[10]++; }
	else
	if( C6_PntsOvrLives[l] <= 1900 && Lives_SDev[l] <= 1.54 && S2a_LivesGained[0] > 0.620 ){ conq += 	0.279529412	; usage_Nve[9]++; usage_Nve[15]++; usage_Nve[29]++; fired[11]++; }
	else
	if( C5_MvsWoutGain[l] > 79 && ChryBool_Sum[l] > 23 && R1_AvgTimeToEatChrry[l] > 36 && A4_HuntAfterPill[l] <= 2 ){ conq -= 	0.278770492	; usage_Nve[8]++; usage_Nve[11]++; usage_Nve[25]++; usage_Nve[2]++; fired[12]--; }
	else
	if( ChryBool_Sum[l] > 23 && S2a_LivesGained[0] > 0.620 && Lives_SDev[l] <= 1.54 ){ conq += 	0.2667	; usage_Nve[11]++; usage_Nve[29]++; usage_Nve[15]++; fired[13]++; }
	else
	if( Lives_SDev[l] > 1.5 ){ conq -= 	0.235264706	; usage_Nve[15]++; fired[14]--; }
	else
	if( A4_HuntAfterPill[l] > 2 && C4_CaughtAfterHunt[l] <= 2 ){ conq += 	0.226733333	; usage_Nve[2]++; usage_Nve[7]++; fired[15]++; }
	else
	if( R2_AvgChrryPerLvl[0] > 0.333 && P7_PutOffGstHse[0] <= 0.292 && C1b_ThreatPerception_NumKilled[l] <= 1 && S2b_LivesLost[0] <= 0.737 ){ conq -= 	0.226133647	; usage_Nve[26]++; usage_Nve[23]++; usage_Nve[4]++; usage_Nve[30]++; fired[16]--; }
	else
	if( Points_Max[l-1] > 1950 && P3_CntDtsB4Pll1[l] > 19 && P4a_HuntGst1_AvgSpd[l] <= 230 ){ conq -= 	0.220705882	; usage_Nve[24]++; usage_Nve[20]++; usage_Nve[21]++; fired[17]--; }
	else
	if( R2_AvgChrryPerLvl[0] > 0.333 && P7_PutOffGstHse[0] <= 0.438 && P7_PutOffGstHse[0] >= 0.292 ){ conq -= 	0.219350147	; usage_Nve[26]++; usage_Nve[23]++; usage_Nve[23]++; fired[18]--; }
	else
	if( D2_Osclltng[l] > 122 ){ conq -= 	0.2133	; usage_Nve[14]++; fired[19]--; }
	else
	if( A4_HuntAfterPill[l] <= 1 ){ conq -= 	0.213213115	; usage_Nve[2]++; fired[20]--; }
	else
	if( C5_MvsWoutGain[l] <= 106 && S2a_LivesGained[0] > 0.620 ){ conq += 	0.213196721	; usage_Nve[8]++; usage_Nve[29]++; fired[21]++; }
	else
	if( A4_HuntAfterPill[l] <= 2 && R2_AvgChrryPerLvl[0] <= 0.802 && A3_GhostKills[l] > 0 ){ conq -= 	0.213114754	; usage_Nve[2]++; usage_Nve[26]++; usage_Nve[1]++; fired[22]--; }
	else
	if( GameOver && A4_HuntAfterPill[l] > 1 && S5_GhostHunts[l] > 3.75 && CycleCountPerState_SDev[l] > 13 ){ conq -= 	0.2	; usage_Nve[2]++; usage_Nve[32]++; usage_Nve[12]++; fired[23]--; }
	else
	if( A4_HuntAfterPill[l] <= 2 && R2_AvgChrryPerLvl[0] <= 0.802 && A3_GhostKills[l] <= 0 && P1c_GstsEtnOnPll[l] <= 1 ){ conq -= 	0.196688525	; usage_Nve[2]++; usage_Nve[26]++; usage_Nve[1]++; usage_Nve[18]++; fired[24]--; }
	else
	if( A4_HuntAfterPill[l] > 0 && A1_AtkGstHse[l] <= 0 && A6_Chase¬Dots[l] <= 5 && P1c_GstsEtnOnPll[l] > 0 ){ conq += 	0.19175	; usage_Nve[2]++; usage_Nve[0]++; usage_Nve[3]++; usage_Nve[18]++; fired[25]++; }
	else
	if( A4_HuntAfterPill[l] >= 1 && S1b_AvgStates[l] <= 304 ){ conq += 	0.191264706	; usage_Nve[2]++; usage_Nve[27]++; fired[26]++; }
	else
	if( A4_HuntAfterPill[l] > 0 && S1b_AvgStates[l] <= 305 && C6_PntsOvrLives[l] <= 1900 ){ conq += 	0.176455882	; usage_Nve[2]++; usage_Nve[27]++; usage_Nve[9]++; fired[27]++; }
	else
	if( GameOver && S1b_AvgStates[l] <= 393 && C4_CaughtAfterHunt[l] > 2 && S5_GhostHunts[l] > 3.75 ){ conq -= 	0.174110588	; usage_Nve[27]++; usage_Nve[7]++; usage_Nve[32]++; fired[28]--; }
	else
	if( A4_HuntAfterPill[l] <= 2 && S2a_LivesGained[0] > 0.627 && Lives_SDev[l] <= 1.55 && P1_LureToPill[l] > 0 ){ conq += 	0.163934426	; usage_Nve[2]++; usage_Nve[29]++; usage_Nve[15]++; usage_Nve[16]++; fired[29]++; }
	else
	if( R2_AvgChrryPerLvl[0] > 0.333 && P7_PutOffGstHse[0] > 0.438 && Lives_SDev[l] <= 1.52 ){ conq += 	0.163206441	; usage_Nve[26]++; usage_Nve[23]++; usage_Nve[15]++; fired[30]++; }
	else
	if( Points_Max[l-1] <= 1895 ){ conq += 	0.161826824	; usage_Nve[24]++; fired[31]++; }
	else
	if( A4_HuntAfterPill[l] > 2 && S1c_AvgCycles[l] <= 5650 ){ conq += 	0.161764706	; usage_Nve[2]++; usage_Nve[28]++; fired[32]++; }
	else
	if( A4_HuntAfterPill[l] <= 2 && S2a_LivesGained[0] > 0.620 && D1_CyclesPerMove[l] <= 15.45 && Lives_SDev[l] <= 1.4 ){ conq += 	0.161764706	; usage_Nve[2]++; usage_Nve[29]++; usage_Nve[13]++; usage_Nve[15]++; fired[33]++; }
	else
	if( P1d_PacEtnB4GetPll[l] <= 0 && C1b_ThreatPerception_NumKilled[l] <= 2 && R2_AvgChrryPerLvl[0] > 0.333 && P1c_GstsEtnOnPll[l] <= 2 && P1b_2GstsLrd2Pll[l] <= 1 && P4a_HuntGst1_AvgSpd[l] <= 224 && R2_AvgChrryPerLvl[0] <= 0.750 && A6_Chase¬Dots[l] > 1 ){ conq -= 	0.161764706	; usage_Nve[19]++; usage_Nve[4]++; usage_Nve[26]++; usage_Nve[18]++; usage_Nve[17]++; usage_Nve[21]++; usage_Nve[26]++; usage_Nve[3]++; fired[34]--; }
	else
	if( A4_HuntAfterPill[l] > 2 ){ conq += 	0.147540984	; usage_Nve[2]++; fired[35]++; }
	else
	if( S1c_AvgCycles[l] > 5500 ){ conq -= 	0.147540984	; usage_Nve[28]++; fired[36]--; }
	else
	if( A4_HuntAfterPill[l] > 1 && ChryBool_Sum[l] <= 23 ){ conq -= 	0.147540984	; usage_Nve[2]++; usage_Nve[11]++; fired[37]--; }
	else
	if( ChryBool_Sum[l] <= 24 ){ conq -= 	0.1333	; usage_Nve[11]++; fired[38]--; }
	else
	if( Points_Max[l-1] <= 1950 && P4b_HuntGst2_AvgSpd[l] <= 38 ){ conq += 	0.132352941	; usage_Nve[24]++; usage_Nve[22]++; fired[39]++; }
	else
	if( R2_AvgChrryPerLvl[0] > 0.333 && P7_PutOffGstHse[0] <= 0.292 && C1b_ThreatPerception_NumKilled[l] > 1 && S4_TeleportUse[l] <= 2 ){ conq += 	0.131411765	; usage_Nve[26]++; usage_Nve[23]++; usage_Nve[4]++; usage_Nve[31]++; fired[40]++; }
	else
	if( A4_HuntAfterPill[l] <= 2 && S2a_LivesGained[0] > 0.627 && Lives_SDev[l] > 1.55 ){ conq -= 	0.131163934	; usage_Nve[2]++; usage_Nve[29]++; usage_Nve[15]++; fired[41]--; }
	else
	if( D2_Osclltng[l] <= 123 && R1_AvgTimeToEatChrry[l] <= 21 ){ conq -= 	0.126575738	; usage_Nve[14]++; usage_Nve[25]++; fired[42]--; }
	else
	if( ChryBool_Sum[l] > 23 && C3_CloseCalls[l] <= 2 ){ conq += 	0.12	; usage_Nve[11]++; usage_Nve[25]++; fired[43]++; }
	else
	if( ChryBool_Sum[l] > 23 && C3_CloseCalls[l] > 0 && A4_HuntAfterPill[l] > 2 ){ conq += 	0.119973333	; usage_Nve[11]++; usage_Nve[6]++; usage_Nve[2]++; fired[44]++; }
	else
	if( S2a_LivesGained[0] <= 0.620 && A4_HuntAfterPill[l] > 1 && A6_Chase¬Dots[l] <= 1 ){ conq += 	0.117647059	; usage_Nve[29]++; usage_Nve[2]++; usage_Nve[3]++; fired[45]++; }
	else
	if( A4_HuntAfterPill[l] > 0 && A1_AtkGstHse[l] > 0 && P1d_PacEtnB4GetPll[l] <= 0 && Lives_SDev[l] <= 1.52 && C3_CloseCalls[l] <= 3 && C7_KilledAtHouse[l] <= 2 ){ conq += 	0.113455882	; usage_Nve[2]++; usage_Nve[0]++; usage_Nve[19]++; usage_Nve[15]++; usage_Nve[6]++; usage_Nve[10]++; fired[46]++; }
	else
	if( C5_MvsWoutGain[l] > 150 ){ conq -= 	0.1067	; usage_Nve[8]++; fired[47]--; }
	else
	if( P7_PutOffGstHse[0] > 0.656 ){ conq += 	0.106509836	; usage_Nve[23]++; fired[48]++; }
	else
	if( ChryBool_Sum[l] > 23 && A6_Chase¬Dots[l] > 5 ){ conq -= 	0.102941176	; usage_Nve[11]++; usage_Nve[3]++; fired[49]--; }
...
//////////////////////////////////////////////////////////////////////////////
//		THE OLD WAY OF DOING IT: USING ALL THE RULES AND SUMMING THEM		//
//////////////////////////////////////////////////////////////////////////////
...
float ClassifyConqueror_WindowAvg( int lvl )
{
	...
	if( GameOver && A4_HuntAfterPill[0] > 0.947175 && S5_GhostHunts[l] > 3.764 && CycleCountPerState_SDev[0] > 13 ){ conq -= 0.2; usage_Wdw[2]++; usage_Wdw[32]++; usage_Wdw[12]++; fired[29]--; }
...
float ClassifyConqueror_Averages( int lvl )
{
	 ...
	 *	0.106509836		Rule 3 Conqr (7.219; 0.9) if P7_PutOffGstHse_Avg > 0.656 then Conqr	// direct map - 0.375
	 */
	if( Lives_SDev[0] > 1.5 ){ conq -= 0.5 -0.235264706; usage_Avg[15]++; fired[0]--; }	//weight[0] -= 0.01; }

	if( D2_Osclltng[0] > 122 ){ conq -= 0.5 -0.2133; usage_Avg[14]++; fired[1]--; }

	if( A4_HuntAfterPill[0] <= 1.16 ){ conq -= 0.5 -0.213213115; usage_Avg[2]++; fired[2]--; }

	if( Points_Max[l-1] <= 1870 ){ conq += 0.5 -0.161826824; usage_Avg[24]++; fired[3]++; }	// Can only fire on last level

	if( A4_HuntAfterPill[0] > 3.08 ){ conq += 0.5 -0.147540984; usage_Avg[2]++; fired[4]++; }

	if( S1c_AvgCycles[0] > 5500 ){ conq -= 0.5 -0.147540984; usage_Avg[28]++; fired[5]--; }

	if( ChryBool_Sum[0] <= 23 ){ conq -= 0.5 -0.1333; usage_Avg[11]++; fired[6]--; }

	if( C5_MvsWoutGain[0] > 150 ){ conq -= 0.5 -0.1067; usage_Avg[8]++; fired[7]--; }

	if( P7_PutOffGstHse[0] > 0.375 ){ conq += 0.5 -0.106509836; usage_Avg[23]++; fired[8]++; }
	 ...
	 *	0.311442623		Rule 1 Not (23; 0.826) if C5_MvsW-outGain_Avg > 0.305 and CloseCalls_Avg > 0.103 and CloseCalls_Avg <= 0.206 then Not
	 */
	if( A4_HuntAfterPill[0] > 2.5185 && C4_CaughtAfterHunt[0] <= 0.69625 ){ conq += 0.5 -0.226733333; usage_Avg[2]++; usage_Avg[7]++; fired[9]++; }

	if( C5_MvsWoutGain[0] <= 106 && S2a_LivesGained[0] > 0.775 ){ conq += 0.5 -0.213196721; usage_Avg[8]++; usage_Avg[29]++; fired[10]++; }

	if( A4_HuntAfterPill[0] >= 0.914 && S1b_AvgStates[0] <= 304 ){ conq += 0.5 -0.191264706; usage_Avg[2]++; usage_Avg[27]++; fired[11]++; }

	if( A4_HuntAfterPill[0] > 3.08 && S1c_AvgCycles[0] <= 5653 ){ conq += 0.5 -0.161764706; usage_Avg[2]++; usage_Avg[28]++; fired[12]++; }

	if( A4_HuntAfterPill[0] > 1.16 && ChryBool_Sum[0] <= 23 ){ conq -= 0.5 -0.147540984; usage_Avg[2]++; usage_Avg[11]++; fired[13]--; }

	if( Points_Max[l-1] <= 1878 && P4b_HuntGst2_AvgSpd[0] <= 39 ){ conq += 0.5 -0.132352941; usage_Avg[24]++; usage_Avg[22]++; fired[14]++; }	// Can only fire on last level

	if( D2_Osclltng[0] <= 123 && R1_AvgTimeToEatChrry[0] <= 21 ){ conq -= 0.5 -0.126575738; usage_Avg[14]++; usage_Avg[25]++; fired[15]--; }

	if( ChryBool_Sum[0] > 23 && C3_CloseCalls[0] <= 0.174 ){ conq += 0.5 -0.12; usage_Avg[11]++; usage_Avg[25]++; fired[16]++; }

	if( ChryBool_Sum[0] > 22 && A6_Chase¬Dots[0] > 5.25 ){ conq -= 0.5 -0.102941176; usage_Avg[11]++; usage_Avg[3]++; fired[17]--; }

	if( C5_MvsWoutGain[0] > 78 && C3_CloseCalls[0] > 0.1545 ){ conq -= 0.5 -0.311442623; usage_Avg[8]++; usage_Avg[6]++; usage_Avg[6]++; fired[18]--; }
	 ...
	 *	0.117647059	Rule 1 Conqr (10; 0.8) if S2.a_LivesGained_Avg <= 0.620 and HuntAfterPill_Avg > 0.257 and Chase¬Dots_Avg <= 0.124 then Conqr
	 */
	if( C5_MvsWoutGain[0] > 106 && P4b_HuntGst2_AvgSpd[0] <= 1163 && P7_PutOffGstHse[0] <= 0.2678 ){ conq -= 0.5 -0.393491803; usage_Avg[8]++; usage_Avg[22]++; usage_Avg[23]++; fired[19]--; }

	if( S2a_LivesGained[0] > 0.775 && S1c_AvgCycles[0] <= 5653 && Lives_SDev[0] <= 1.674 ){ conq += 0.5 -0.327836066; usage_Avg[29]++; usage_Avg[28]++; usage_Avg[15]++; fired[20]++; }

	if( A4_HuntAfterPill[0] <= 3.08 && S2a_LivesGained[0] <= 0.78375 && Lives_SDev[0] <= 1.468 ){ conq -= 0.5 -0.327737705; usage_Avg[2]++; usage_Avg[29]++; usage_Avg[15]++; fired[21]--; }

	if( S2a_LivesGained[0] > 0.775 && C4_CaughtAfterHunt[0] <= 0.69625 && S1c_AvgCycles[0] <= 6527 ){ conq += 0.5 -0.323647059; usage_Avg[29]++; usage_Avg[7]++; usage_Avg[28]++; fired[22]++; }

	if( S1c_AvgCycles[0] <= 4396 && A4_HuntAfterPill[0] > 0.99 && S2a_LivesGained[0] > 0.53625 ){ conq += 0.5 -0.294117647; usage_Avg[28]++; usage_Avg[2]++; usage_Avg[29]++; fired[23]++; }

	if( C6_PntsOvrLives[0] <= 1897 && Lives_SDev[0] <= 1.54 && S2a_LivesGained[0] > 0.775 ){ conq += 0.5 -0.279529412; usage_Avg[9]++; usage_Avg[15]++; usage_Avg[29]++; fired[24]++; }

	if( ChryBool_Sum[0] > 23 && S2a_LivesGained[0] > 0.775 && Lives_SDev[0] <= 1.53 ){ conq += 0.5 -0.2667; usage_Avg[11]++; usage_Avg[29]++; usage_Avg[15]++; fired[25]++; }

	if( Points_Max[l-1] > 1950 && P3_CntDtsB4Pll1[0] > 4 && P4a_HuntGst1_AvgSpd[0] <= 236 ){ conq -= 0.5 -0.220705882; usage_Avg[24]++; usage_Avg[20]++; usage_Avg[21]++; fired[26]--; }

	//This rule selects play where PutOffGstHse occurs between 1 sixth and 1 quarter times per level - can only be measured over 6 or more levels.
	if( R2_AvgChrryPerLvl[0] > 0.333 && P7_PutOffGstHse[0] <= 0.250098 && P7_PutOffGstHse[0] >= 0.166732 ){ conq -= 0.5 -0.219350147; usage_Avg[26]++; usage_Avg[23]++; usage_Avg[23]++; fired[27]--; }

	if( A4_HuntAfterPill[0] <= 3.08 && R2_AvgChrryPerLvl[0] <= 0.802 && A3_GhostKills[0] > 0.46 ){ conq -= 0.5 -0.213114754; usage_Avg[2]++; usage_Avg[26]++; usage_Avg[1]++; fired[28]--; }

	//This rule's 2nd condition can only NOT fire on the last level, depending on whether Pacman dies after 3rd or fourth Pill - but we let it fire anyway to balance available points between Conq & Not.
	if( GameOver && A4_HuntAfterPill[0] > 0.947175 && S5_GhostHunts[l] > 3.764 && CycleCountPerState_SDev[0] > 13 ){ conq -= 0.5 -0.2; usage_Avg[2]++; usage_Avg[32]++; usage_Avg[12]++; fired[29]--; }

	if( A4_HuntAfterPill[0] > 0.7446 && S1b_AvgStates[0] <= 305 && C6_PntsOvrLives[0] <= 1912 ){ conq += 0.5 -0.176455882; usage_Avg[2]++; usage_Avg[27]++; usage_Avg[9]++; fired[30]++; }

	//This rule's 2nd condition can only NOT fire on the last level, depending on whether Pacman dies after 3rd or fourth Pill - so we only let it fire on the last level.
	if( GameOver && S1b_AvgStates[0] <= 394 && C4_CaughtAfterHunt[0] > 0.445 && S5_GhostHunts[l] > 3.644 ){ conq -= 0.5 -0.174110588; usage_Avg[27]++; usage_Avg[7]++; usage_Avg[32]++; fired[31]--; }

	if( R2_AvgChrryPerLvl[0] > 0.333 && P7_PutOffGstHse[0] > 0.250098 && Lives_SDev[0] <= 1.52 ){ conq += 0.5 -0.163206441; usage_Avg[26]++; usage_Avg[23]++; usage_Avg[15]++; fired[32]++; }

	if( A4_HuntAfterPill[0] <= 3.08 && S2a_LivesGained[0] > 0.78375 && Lives_SDev[0] > 1.55 ){ conq -= 0.5 -0.131163934; usage_Avg[2]++; usage_Avg[29]++; usage_Avg[15]++; fired[33]--; }

	if( ChryBool_Sum[0] > 23 && C3_CloseCalls[0] > 0.174 && A4_HuntAfterPill[0] > 3.104325 ){ conq += 0.5 -0.119973333; usage_Avg[11]++; usage_Avg[6]++; usage_Avg[2]++; fired[34]++; }

	if( S2a_LivesGained[0] <= 0.775 && A4_HuntAfterPill[0] > 1.407075 && A6_Chase¬Dots[0] <= 1.447 ){ conq += 0.5 -0.117647059; usage_Avg[29]++; usage_Avg[2]++; usage_Avg[3]++; fired[35]++; }
	 ...
	 */
	if( A4_HuntAfterPill[0] > 0.914325 && S1b_AvgStates[0] > 304 && C3_CloseCalls[0] > 0.1665 && S4_TeleportUse[0] > 0.858 )
	{ conq -= 0.5 -0.374970588; usage_Avg[2]++; usage_Avg[27]++; usage_Avg[6]++; usage_Avg[31]++; fired[36]--; }

	if( C5_MvsWoutGain[0] > 106 && P7_PutOffGstHse[0] <= 0.2678 && C1b_ThreatPerception_NumKilled[0] <= 2.5 && A4_HuntAfterPill[0] <= 2.9346 )
	{ conq -= 0.5 -0.34442623; usage_Avg[8]++; usage_Avg[23]++; usage_Avg[4]++; usage_Avg[2]++; fired[37]--; }

	if( Lives_SDev[0] <= 1.54 && A4_HuntAfterPill[0] > 1.14975 && CycleCountPerState_SDev[0] <= 15.7575 && C4_CaughtAfterHunt[0] <= 0.45 )
	{ conq += 0.5 -0.294264706; usage_Avg[15]++; usage_Avg[2]++; usage_Avg[12]++; usage_Avg[7]++; fired[38]++; }

	//This rule's 2nd condition can ONLY fire on the last level, depending on whether Pacman dies after 3rd or fourth Pill - so we only let it fire on the last level.
	if( GameOver && A4_HuntAfterPill[0] > 0.947175 && S5_GhostHunts[l] <= 3.764 && A6_Chase¬Dots[0] <= 5.3 && ChryBool_Sum[0] > 24 )
	{ conq += 0.5 -0.2933; usage_Avg[2]++; usage_Avg[32]++; usage_Avg[3]++; usage_Avg[11]++; fired[39]++; }

	if( C5_MvsWoutGain[0] > 78 && ChryBool_Sum[0] > 23 && R1_AvgTimeToEatChrry[0] > 36 && A4_HuntAfterPill[0] <= 2.4966 )
	{ conq -= 0.5 -0.278770492; usage_Avg[8]++; usage_Avg[11]++; usage_Avg[25]++; usage_Avg[2]++; fired[40]--; }

	if( R2_AvgChrryPerLvl[0] > 0.333 && P7_PutOffGstHse[0] <= 0.166732 && C1b_ThreatPerception_NumKilled[0] <= 1 && S2b_LivesLost[0] <= 0.737 )
	{ conq -= 0.5 -0.226133647; usage_Avg[26]++; usage_Avg[23]++; usage_Avg[4]++; usage_Avg[30]++; fired[41]--; }

	if( A4_HuntAfterPill[0] <= 3.08 && R2_AvgChrryPerLvl[0] <= 0.802 && A3_GhostKills[0] <= 0.46 && P1c_GstsEtnOnPll[0] <= 0.6588 )
	{ conq -= 0.5 -0.196688525; usage_Avg[2]++; usage_Avg[26]++; usage_Avg[1]++; usage_Avg[18]++; fired[42]--; }

	if( A4_HuntAfterPill[0] > 0.914325 && A1_AtkGstHse[0] <= 0.4288 && A6_Chase¬Dots[0] <= 4.843 && P1c_GstsEtnOnPll[0] > 0.5 )
	{ conq += 0.5 -0.19175; usage_Avg[2]++; usage_Avg[0]++; usage_Avg[3]++; usage_Avg[18]++; fired[43]++; }

	if( A4_HuntAfterPill[0] <= 3.08 && S2a_LivesGained[0] > 0.78375 && Lives_SDev[0] <= 1.55 && P1_LureToPill[0] > 0.08277 )
	{ conq += 0.5 -0.163934426; usage_Avg[2]++; usage_Avg[29]++; usage_Avg[15]++; usage_Avg[16]++; fired[44]++; }

	if( A4_HuntAfterPill[0] <= 3.08 && S2a_LivesGained[0] > 0.775 && D1_CyclesPerMove[0] <= 15.6 && Lives_SDev[0] <= 1.4 )
	{ conq += 0.5 -0.161764706; usage_Avg[2]++; usage_Avg[29]++; usage_Avg[13]++; usage_Avg[15]++; fired[45]++; }

	if( R2_AvgChrryPerLvl[0] > 0.333 && P7_PutOffGstHse[0] <= 0.166732 && C1b_ThreatPerception_NumKilled[0] > 1 && S4_TeleportUse[0] <= 1.923 )
	{ conq += 0.5 -0.131411765; usage_Avg[26]++; usage_Avg[23]++; usage_Avg[4]++; usage_Avg[31]++; fired[46]++; }
...
float ClassifyConqueror_Naive( int lvl )
{
	...
	if( GameOver && S1b_AvgStates[l] <= 393 && C4_CaughtAfterHunt[l] > 2 && S5_GhostHunts[l] > 3.75 ){ conq -= 0.174110588; usage_Nve[27]++; usage_Nve[7]++; usage_Nve[32]++; fired[31]--; }
	...
	if( GameOver && A4_HuntAfterPill[l] > 1 && S5_GhostHunts[l] <= 3.75 && A6_Chase¬Dots[l] <= 5 && ChryBool_Sum[l] > 25 )
...
naive_rules.push_back( "if( Points_Max <= 1895 )then Conq\n" );
naive_rules.push_back( "if( A4_HuntAfterPill > 2 )then Conq\n" );
...
naive_rules.push_back( "if( P7_PutOffGstHse == 1 )then Conq\n" );
naive_rules.push_back( "if( A4_HuntAfterPill > 2  and  C4_CaughtAfterHunt <= 2 )then Conq\n" );
naive_rules.push_back( "if( C5_MvsWoutGain <= 106  and  S2a_LivesGained == 1 )then Conq\n" );
naive_rules.push_back( "if( A4_HuntAfterPill >= 1  and  S1b_AvgStates <= 304 )then Conq\n" );
naive_rules.push_back( "if( A4_HuntAfterPill > 2  and  S1c_AvgCycles <= 5650 )then Conq\n" );
naive_rules.push_back( "if( A4_HuntAfterPill > 1  and  ChryBool_Sum <= 23 )then NotCq\n" );
naive_rules.push_back( "if( Points_Max <= 1950  and  P4b_HuntGst2_AvgSpd <= 38 )then Conq\n" );
naive_rules.push_back( "if( D2_Osclltng <= 123  and  R1_AvgTimeToEatChrry <= 21 )then NotCq\n" );
naive_rules.push_back( "if( ChryBool_Sum > 23  and  C3_CloseCalls <= 2 )then Conq\n" );
...
naive_rules.push_back( "if( S2a_LivesGained == 1  and  S1c_AvgCycles <= 5650  and  Lives_SDev <= 1.654 )then Conq\n" );
naive_rules.push_back( "if( A4_HuntAfterPill <= 2  and  S2a_LivesGained == 0  and  Lives_SDev <= 1.469 )then NotCq\n" );
naive_rules.push_back( "if( S2a_LivesGained == 1  and  C4_CaughtAfterHunt <= 2  and  S1c_AvgCycles <= 6530 )then Conq\n" );
naive_rules.push_back( "if( S1c_AvgCycles <= 4410  and  A4_HuntAfterPill > 1  and  S2a_LivesGained == 1 )then Conq\n" );
naive_rules.push_back( "if( C6_PntsOvrLives <= 1900  and  Lives_SDev <= 1.54  and  S2a_LivesGained == 1 )then Conq\n" );
naive_rules.push_back( "if( ChryBool_Sum > 23  and  S2a_LivesGained == 1  and  Lives_SDev <= 1.54 )then Conq\n" );
...
naive_rules.push_back( "if( A4_HuntAfterPill > 0  and  S1b_AvgStates <= 305  and  C6_PntsOvrLives <= 1900 )then Conq\n" );
naive_rules.push_back( "if( S1b_AvgStates <= 393  and  C4_CaughtAfterHunt > 2  and  S5_GhostHunts > 3.75 )then NotCq\n" );
naive_rules.push_back( "if( R2_AvgChrryPerLvl == 1  and  P7_PutOffGstHse == 1  and  Lives_SDev <= 1.52 )then Conq\n" );
naive_rules.push_back( "if( A4_HuntAfterPill <= 2  and  S2a_LivesGained == 1  and  Lives_SDev > 1.55 )then NotCq\n" );
naive_rules.push_back( "if( ChryBool_Sum > 23  and  C3_CloseCalls > 0  and  A4_HuntAfterPill > 2 )then Conq\n" );
naive_rules.push_back( "if( S2a_LivesGained == 0  and  A4_HuntAfterPill > 1  and  A6_Chase¬Dots <= 1 )then Conq\n" );
...
naive_rules.push_back( "if( Lives_SDev <= 1.54  and  A4_HuntAfterPill > 1  and  CycleCountPerState_SDev <= 15.77  and  C4_CaughtAfterHunt <= 1 )then Conq\n" );
naive_rules.push_back( "if( A4_HuntAfterPill > 1  and  S5_GhostHunts <= 3.75  and  A6_Chase¬Dots <= 5  and  ChryBool_Sum > 25 )then Conq\n" );
...
naive_rules.push_back( "if( A4_HuntAfterPill > 0  and  A1_AtkGstHse <= 0  and  A6_Chase¬Dots <= 5  and  P1c_GstsEtnOnPll > 0 )then Conq\n" );
naive_rules.push_back( "if( A4_HuntAfterPill <= 2  and  S2a_LivesGained == 1  and  Lives_SDev <= 1.55  and  P1_LureToPill > 0 )then Conq\n" );
naive_rules.push_back( "if( A4_HuntAfterPill <= 2  and  S2a_LivesGained == 1  and  D1_CyclesPerMove <= 15.45  and  Lives_SDev <= 1.4 )then Conq\n" );
naive_rules.push_back( "if( R2_AvgChrryPerLvl == 1  and  P7_PutOffGstHse == 0  and  C1b_ThreatPerception_NumKilled > 1  and  S4_TeleportUse <= 2 )then Conq\n" );
...
naive_rules.push_back( "if( A4_HuntAfterPill > 0  and  A1_AtkGstHse > 0  and  P1d_PacEtnB4GetPll <= 0  and  Lives_SDev <= 1.52  and  C3_CloseCalls <= 3  and  C7_KilledAtHouse <= 2 )then Conq\n" );
naive_rules.push_back( "if( C5_MvsWoutGain <= 150  and  A4_HuntAfterPill > 0  and  C2b_AvgDstPacAtkCntrd <= 13.26  and  C4_CaughtAfterHunt <= 2  and  ChryBool_Sum > 23 )then Conq\n" );

ord_rules.push_back( "Conqr (45; 0.756)  if C5_MvsW-outGain_Avg <= 0.584 and HuntAfterPill_Avg > 0.171 and C2.b_AvgDstPacAtk-Cntrd <= 0.941 and C4_CaughtAfterHunt_Avg <= 0.554 and ChryBool_Sum_Avg > 0.136 then Conqr\n" );
ord_rules.push_back( "Not (27; 0.889) if C5_MvsW-outGain_Avg > 0.413 and HuntGst2_AvgSpd <= 0.504 and P7_PutOffGstHse_Avg <= 0.469 then Not\n" );
ord_rules.push_back( "Not (30.5; 0.836)  if HuntAfterPill_Avg > 0.167 and S1.b_AvgStates > 0.636 and CloseCalls_Avg > 0.111 and S4_TeleportUse_Avg > 0.286 then Not\n" );
ord_rules.push_back( "Not (22; 0.955) if C5_MvsW-outGain_Avg > 0.413 and P7_PutOffGstHse_Avg <= 0.469 and C1.b_ThreatPrcptn-#Killed <= 0.417 and HuntAfterPill_Avg <= 0.536 then Not\n" );
ord_rules.push_back( "Conqr (22; 0.909)  if S2.a_LivesGained_Avg > 0.620 and S1.c_AvgCycles <= 0.589 and Lives_SDev <= 0.659 then Conqr\n" );
ord_rules.push_back( "Not (21; 0.952) if HuntAfterPill_Avg <= 0.563 and S2.a_LivesGained_Avg <= 0.627 and Lives_SDev <= 0.578 then Not\n" );
ord_rules.push_back( "Conqr (28; 0.786) if S2.a_LivesGained_Avg > 0.620 and C4_CaughtAfterHunt_Avg <= 0.557 and S1.c_AvgCycles <= 0.680 then Conqr\n" );
ord_rules.push_back( "Not (23; 0.826) if C5_MvsW-outGain_Avg > 0.305 and CloseCalls_Avg > 0.103 and CloseCalls_Avg <= 0.206 then Not\n" );
ord_rules.push_back( "Conqr (23; 0.87)  if Lives_SDev <= 0.602 and HuntAfterPill_Avg > 0.210 and CycleCountPerState_SDev <= 0.275 and C4_CaughtAfterHunt_Avg <= 0.360 then Conqr\n" );
ord_rules.push_back( "Conqr (25; 0.8)  if S1.c_AvgCycles <= 0.458 and HuntAfterPill_Avg > 0.181 and S2.a_LivesGained_Avg > 0.429 then Conqr\n" );
ord_rules.push_back( "Conqr (25; 0.88)  if HuntAfterPill_Avg > 0.173 and GhostHunts_Avg <= 0.941 and Chase¬Dots_Avg <= 0.454 and ChryBool_Sum_Avg > 0.144 then Conqr\n" );
ord_rules.push_back( "Conqr (22; 0.864) if C6_PntsOvrLives <= 0.506 and Lives_SDev <= 0.603 and S2.a_LivesGained_Avg > 0.620 then Conqr\n" );
ord_rules.push_back( "Not (19; 0.895)  if C5_MvsW-outGain_Avg > 0.305 and ChryBool_Sum_Avg > 0.137 and R1_AvgTimeToEatChrry > 0.257 and HuntAfterPill_Avg <= 0.456 then Not\n" );
ord_rules.push_back( "Conqr (25; 0.8)  if ChryBool_Sum_Avg > 0.137 and S2.a_LivesGained_Avg > 0.620 and Lives_SDev <= 0.602 then Conqr\n" );
ord_rules.push_back( "Not (19; 0.842)  if Lives_SDev > 0.597 then Not\n" );
ord_rules.push_back( "Conqr (19; 0.895) if HuntAfterPill_Avg > 0.460 and C4_CaughtAfterHunt_Avg <= 0.557 then Conqr\n" );
ord_rules.push_back( "Not (18.112; 0.849)  if R2_AvgChrryPerLvl > 0.333 and P7_PutOffGstHse_Avg <= 0.438 and P7_PutOffGstHse_Avg <= 0.292 and C1.b_ThreatPrcptn-#Killed <= 0.167 and S2.b_LivesLost <= 0.737 then Not\n" );
ord_rules.push_back( "Not (16; 0.938)  if Points_Max_Avg > 0.448 and P4_CntDtsb4Pll1_Avg > 0.191 and HuntGst1_AvgSpd <= 0.676 then Not\n" );
ord_rules.push_back( "Not (16.391; 0.91) if R2_AvgChrryPerLvl > 0.333 and P7_PutOffGstHse_Avg <= 0.438 and P7_PutOffGstHse_Avg > 0.292 then Not\n" );
ord_rules.push_back( "Not (14.22; 0.915) if D2_Osclltng_Avg > 0.678 then Not\n" );
ord_rules.push_back( "Not (14; 0.929)  if HuntAfterPill_Avg <= 0.212 then Not\n" );
ord_rules.push_back( "Conqr (15; 0.867)  if C5_MvsW-outGain_Avg <= 0.413 and S2.a_LivesGained_Avg > 0.620 then Conqr\n" );
ord_rules.push_back( "Not (13; 1.0) if HuntAfterPill_Avg <= 0.563 and R2_AvgChrryPerLvl <= 0.802 and GhostKills_Avg > 0.378 then Not\n" );
ord_rules.push_back( "Not (15; 1.0) if HuntAfterPill_Avg > 0.173 and GhostHunts_Avg > 0.941 and CycleCountPerState_SDev > 0.229 then Not\n" );
ord_rules.push_back( "Not (14; 0.857)  if HuntAfterPill_Avg <= 0.563 and R2_AvgChrryPerLvl <= 0.802 and GhostKills_Avg <= 0.378 and P1.c_GstsEtnOnPll_Avg <= 0.183 then Not\n" );
ord_rules.push_back( "Conqr (13.039; 1.0)  if HuntAfterPill_Avg > 0.167 and A1_AtkGstHse_Avg <= 0.134 and Chase¬Dots_Avg <= 0.415 and P1.c_GstsEtnOnPll_Avg > 0.139 then Conqr\n" );
ord_rules.push_back( "Conqr (14; 0.929)  if HuntAfterPill_Avg > 0.167 and S1.b_AvgStates <= 0.636 then Conqr\n" );
ord_rules.push_back( "Conqr (13; 0.923) if HuntAfterPill_Avg > 0.136 and S1.b_AvgStates <= 0.639 and C6_PntsOvrLives <= 0.510 then Conqr\n" );
ord_rules.push_back( "Not (13.64; 0.868)  if S1.b_AvgStates <= 0.824 and C4_CaughtAfterHunt_Avg > 0.356 and GhostHunts_Avg > 0.911 then Not\n" );
ord_rules.push_back( "Conqr (10; 1.0) if HuntAfterPill_Avg <= 0.563 and S2.a_LivesGained_Avg > 0.627 and Lives_SDev <= 0.609 and P1_LureToPill_Avg > 0.031 then Conqr\n" );
ord_rules.push_back( "Conqr (13.307; 0.834)  if R2_AvgChrryPerLvl > 0.333 and P7_PutOffGstHse_Avg > 0.438 and Lives_SDev <= 0.597 then Conqr\n" );
ord_rules.push_back( "Conqr (12.992; 0.847) if Points_Max_Avg <= 0.446 then Conqr\n" );
ord_rules.push_back( "Conqr (11; 1.0) if HuntAfterPill_Avg <= 0.563 and S2.a_LivesGained_Avg > 0.620 and D1_Cycles/Move <= 0.368 and Lives_SDev <= 0.551 then Conqr\n" );
ord_rules.push_back( "Conqr (11; 1.0) if HuntAfterPill_Avg > 0.563 and S1.c_AvgCycles <= 0.589 then Conqr\n" );
ord_rules.push_back( "Not (11; 1.0)  if P1.e_PacEtnB4GetPll_Avg <= 0 and C1.b_ThreatPrcptn-#Killed <= 0.333 and R2_AvgChrryPerLvl > 0.333 and P1.c_GstsEtnOnPll_Avg <= 0.598 and P1.b_2GstsLrd2Pll_Avg <= 0.167 and HuntGst1_AvgSpd <= 0.640 and R2_AvgChrryPerLvl <= 0.750 and Chase¬Dots_Avg > 0.120 then Not\n" );
ord_rules.push_back( "Conqr (9; 1.0) if HuntAfterPill_Avg > 0.563 then Conqr\n" );
ord_rules.push_back( "Not (10; 0.9) if HuntAfterPill_Avg > 0.212 and ChryBool_Sum_Avg <= 0.138 then Not\n" );
ord_rules.push_back( "Not (10; 0.9) if S1.c_AvgCycles > 0.573 then Not\n" );
ord_rules.push_back( "Not (10; 1.0) if ChryBool_Sum_Avg <= 0.137 then Not\n" );
ord_rules.push_back( "Conqr (9; 1.0) if Points_Max_Avg <= 0.448 and HuntGst2_AvgSpd <= 0.017 then Conqr\n" );
ord_rules.push_back( "Conqr (8.936; 1.0)  if R2_AvgChrryPerLvl > 0.333 and P7_PutOffGstHse_Avg <= 0.438 and P7_PutOffGstHse_Avg <= 0.292 and C1.b_ThreatPrcptn-#Killed > 0.167 and S4_TeleportUse_Avg <= 0.641 then Conqr\n" );
ord_rules.push_back( "Not (9; 0.889)  if HuntAfterPill_Avg <= 0.563 and S2.a_LivesGained_Avg > 0.627 and Lives_SDev > 0.609 then Not\n" );
ord_rules.push_back( "Not (8.56; 0.902)  if D2_Osclltng_Avg <= 0.678 and R1_AvgTimeToEatChrry <= 0.145 then Not\n" );
ord_rules.push_back( "Conqr (10; 0.9)  if ChryBool_Sum_Avg > 0.137 and CloseCalls_Avg <= 0.116 then Conqr\n" );
ord_rules.push_back( "Conqr (11; 0.818)  if ChryBool_Sum_Avg > 0.137 and CloseCalls_Avg > 0.116 and HuntAfterPill_Avg > 0.567 then Conqr\n" );
ord_rules.push_back( "Conqr (10; 0.8) if S2.a_LivesGained_Avg <= 0.620 and HuntAfterPill_Avg > 0.257 and Chase¬Dots_Avg <= 0.124 then Conqr\n" );
ord_rules.push_back( "Conqr (7.715; 1.0)  if HuntAfterPill_Avg > 0.167 and A1_AtkGstHse_Avg > 0.134 and P1.e_PacEtnB4GetPll_Avg <= 0 and Lives_SDev <= 0.597 and CloseCalls_Avg <= 0.359 and C7_KilledAtHouse_Avg <= 0.148 then Conqr\n" );
ord_rules.push_back( "Not (8; 1.0) if C5_MvsW-outGain_Avg > 0.584 then Not\n" );
ord_rules.push_back( "Conqr (7.219; 0.9) if P7_PutOffGstHse_Avg > 0.656 then Conqr\n" );
ord_rules.push_back( "Not (7; 1.0) if ChryBool_Sum_Avg > 0.136 and Chase¬Dots_Avg > 0.450 then Not\n" );
