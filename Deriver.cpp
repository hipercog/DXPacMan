...
DO increment game state
	WHILE pacman distance to any ghost LT 6
		IF pacman location changes from one side of the map to the other
			increment output
		ENDIF
	ENDWHILE
...
/**
 *	Description: Count times when Pacman attacks the ghosts and follows them 
...
DO increment game state
	IF play mode EQUAL power pill AND all ghosts distance to ghost house LT 3
		WHILE play mode EQUAL power pill
			...
			IF pacman distance to ghost house LT 3
				increment output
				ENDWHILE
			ENDIF
		ENDWHILE
...
float Aggression1_AtkGstHse( deque<PacState> arg )
{
	...
	for( int i = 1; i < s; i++ )
	{
		...
		if( pS.PacAttack && 
			( myHeuristicDist( pS.xGhost[0], pS.yGhost[0], 9, yHse ) < 3 || 
			myHeuristicDist( pS.xGhost[1], pS.yGhost[1], 9, yHse ) < 3 ))
		{
...
float Aggression6_Chase¬Dots( deque<PacState> arg )
{
	...
	for( int i = 10; i < s; i++ )
	{
		...
		if( arg[i-2].PacAttack == true && arg[i].PacAttack == false && pill < 3 )
			pill++;
	}
	pill = 0;
	for( int i = 0; i < 4; i++ )
...
DO increment game state
	IF remaining power pills GT 0 AND pacman distance to closest power pill LT 8 AND pacman distance to each ghost GT 8
		WHILE play mode EQUAL normal AND pacman distance to closest power pill LT 8
			increment game state
			add pacman movement since last state to first output
		ENDWHILE
		IF play mode EQUAL power pill
			increment second output for each ghost distance to pacman LT 8
			WHILE play mode EQUAL power pill
				IF ghost is caught by pacman
					increment third output
				ENDIF
			ENDWHILE
		ELSE IF pacman lost a life
			increment fourth output
		ENDIF
	ENDIF
UNTIL last game state
 */
...
CREATE 2 outputs
DO increment game state
	IF play mode EQUAL normal AND ghosts block all A* paths from pacman to opposite point on map
		increment first output
		IF pacman lives > pacman lives after 10 game states
	...
	for( int i = 1; i < s; i++ )
	{
			...
			manhattanDist( pS.xPac, pS.yPac, pS.xGhost[1], pS.yGhost[1] ) < 5 )
		{
			...
			if( a_Star(pS.xPac, pS.yPac, 20-pS.xPac, 20-pS.yPac, true, pS) < 6 )
			{
				...
				if( pS.Lives > arg[ min(s-1, i+10) ].Lives )
					output[1]++;
			}
		}
	}
...
float Caution4_CaughtAfterHunt( deque<PacState> arg )
{
	...
	for( int i = 10; i < s; i++ )
	{
		...
		if( arg[i - 1].PacAttack && !pS.PacAttack )
		{
			...
			count = 0;
			j = i;
			while( arg[j].Lives == arg[j - 1].Lives && j < s-1 )
			{
				count++;
				j++;
...
CREATE 1 output
DO increment game state
	WHILE pacman distance to each ghost GT 9
		increment game state
		IF pacman visit count to same location GT 2
			increment output
			ENDWHILE
		ENDIF
	ENDWHILE
...
CREATE 2 outputs
DO increment game state
	IF cherry is onscreen
		WHILE cherry is onscreen
			increment game state
			count states
		ENDWHILE
		IF pacman is closer to cherry position than any ghost
			add count of states while cherry was onscreen to first output
			increment second output
		ENDIF
	ENDIF
UNTIL last game state
...
DO increment game state
	...
	GET pacman location in grid
	WHILE pacman stays at grid location
		increment game state
	ENDWHILE
...
/**
 *	Description: Average number of cycles it takes the player to clear a sector

CREATE 1 output
DO increment game state
	count computer clock cycles
	count game states
	set output as sum of clock cycles divided by sum of states
UNTIL last game state

 */
float Speed1_CyclesPerSector( deque<PacState> arg )
{
	float output = 0.0;
	int s = arg.size();

	//divide number of cycles/number of states while traversing one sector
