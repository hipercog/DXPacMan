...
void EditField();
void XtractRemoteInfo( char* path );
void ExtractInfo();
...
/** /
	GenerateFeatures();
/** /
	XtractRemoteInfo( (char*)(void*)System::Runtime::InteropServices::Marshal::StringToHGlobalAnsi( args[0] ) );
/** /
	ExtractInfo();
/**/
...
/* This is for writing results to a single file * /
	StreamWriter^ allGames = File::CreateText( ".\\_NewDTPMResults.csv" );
	allGames->WriteLine( ",,1st Weights,2nd Weights,Time:ms,States" );
/**/
	for( int i = 0; i < dirs->Length; i++ )
	{
		String^ end = Path::GetFileName( dirs[i] );

		if( dirs[i]->Compare( end, "_Ben" ) == 0 ) //dirs[i]->Compare( end, "c_Conqueror" ) == 0 || dirs[i]->Compare( end, "a_Not_Conqr" ) == 0 || dirs[i]->Compare( end, "b_Unclear" ) == 0 )
		{
...
/** /
				allGames->Write( names[n]->Substring( names[n]->LastIndexOf( '\\' ) ) );
/**/
...
/*	Code for obtaining the final DTPM results of all games * /
						for( int f = 0; f < files->Length; f++ )
						{
							String^ sect = Path::GetFileName( files[f] );
							array <String^> ^dtpm_lines;
							if( sect->Contains( "dp-4,bt-6,A-90,ID-0_09-06" ) )
							{
								dtpm_lines = File::ReadAllLines( files[f] );
								int empties = 3;
								for( int j = 0; j < dtpm_lines->Length; j++ )
									if( dtpm_lines[j]->Contains( "EMPTY NODE" ) )
										empties++;

								allGames->Write( L",Game #" + t + ":," );

								String^ accs = dtpm_lines[ dtpm_lines->Length-3 ];
								allGames->Write( accs->Substring( accs->IndexOf( "%Correct:" )+10, 8 ) + accs->Substring( accs->LastIndexOf( "%Correct2:" )+10, 9 ) );
								accs = dtpm_lines[ dtpm_lines->Length-1 ];
								allGames->WriteLine( String::Concat( accs->Substring( accs->IndexOf( ":" )+1 ), L",", dtpm_lines->Length-empties ) );
							}
						}
/*	Code for applying the processes of results derivation from Level files */
						bool compile = true;
						for( int f = 0; f < files->Length; f++ )
						{
						...
						if( compile )
						{
							ExecuteProcess( L".\\_ManipPacResults_CompileAtPath.exe", times[t], 30 );
						}
						ExecuteProcess( L".\\DX9PacMan.exe", times[t], 120 );
//						ExecuteProcess( L".\\ManipPacResults_XtractNewer.exe", times[t], 30 );
/**/
					}
				}
//				allGames->Write( "\n\n" );
...
/*	Closing statements for the StreamWriter object that compiles all results in one file * /
	allGames->Flush();
	allGames->Close();
/**/
...
/**/
	string xtract = path;
	string time = xtract.substr( xtract.size() - 15, xtract.size()-1 );
	xtract += "\\";
	xtract += time;
	xtract += "_avgNve_ByRank.csv";
	fopen_s(&outf, xtract.c_str(), "wt");

	FILE *states;
	string level = path;
	level += "\\avgNve_ByRank_Csfy-1.csv";
	int count = 2;
	int a = -1, b = -1, c = -1;
	string state("");
	deque <string> csv;
...
/**/
		if( csv[i].compare(0, 7, "Conqr (") == 0 || csv[i].compare(0, 5, "Not (") == 0 )
		{
			state += csv[i];
			state += ",";
			if( csv[i+1].compare( 0, 5, "Score" ) == 0 )
			{
				state += csv[i+4];
				state += ",";
				if( lines < 4 )
...
/**/
		...
		{
			if( lines < 4 && !state.empty() ) state.resize( state.size()-3 );
...
/**/
	String^ plyr = "_someAvgsSingle.csv";
	char* player = (char*)(void*)System::Runtime::InteropServices::Marshal::StringToHGlobalAnsi(plyr);
	fopen_s(&outf, player, "wt");

	FILE *states;
	string level = "avgNve_ByRank_Csfy-1.csv";
	int count = 2;
	int a = -1, b = -1, c = -1;
	string state("");
	deque <string> csv;
	...
	{
/**/
		if( csv[i].compare(0, 3, "if(") == 0 )
		{
			sprintf_s(out, 99, "%d", lvls);
			state += out;
			state += ",";
			state += csv[i];
			state += "\n";
