...
/** /
	GenerateFeatures();
/** /
	ExtractInfo( (char*)(void*)System::Runtime::InteropServices::Marshal::StringToHGlobalAnsi( args[0] ) );
/**/
...
/**/
	...
	for( int i = 0; i < dirs->Length; i++ )
	{
		...
		if( dirs[i]->Compare( end, "Conqueror" ) == 0 || dirs[i]->Compare( end, "Not_Conqr" ) == 0 || dirs[i]->Compare( end, "Unclear" ) == 0 )
		{
			...
			{
/*	Just rename some files (copy & delete) at the player directory level * /
				if( File::Exists( String::Concat( names[n], "\\zAllResults.csv" ) ) )
				{
					File::Copy( String::Concat( names[n], "\\zAllResults.csv" ), String::Concat( names[n], names[n]->Substring( names[n]->LastIndexOf( '\\' ) ), ".csv" ) ); 
					File::Delete( String::Concat( names[n], "\\zAllResults.csv" ) );
				}
//				array <String^> ^times = Directory::GetDirectories( names[n] );
/* This is for writing results to a single file * /
				StreamWriter^ allGames = File::CreateText( String::Concat( names[n], "\\zAllResults.csv" ) );
/** /
...
/*	Code for obtaining the full results of all games * /
						for( int f = 0; f < files->Length; f++ )
						{
							String^ sect = Path::GetFileName( files[f] );
							if( sect->Contains( "_oldNve_InvRank" ) || sect->Contains( "_avgNve_ByRank" ) )
							{
								allGames->Write( File::ReadAllText( files[f] ) );
								allGames->Write( "\n\n" );
							}
						}
/*	Code for applying the processes of results derivation from Level files * /
						...
						ExecuteProcess( L".\\DX9PacMan_avgNve_ByRank.exe", times[t], 120 );
